  ...
  public class IntField : Field
   {
    ...
    public IntField(string name, int value_) :
      base(name)
     {
      fieldValue = value_;
     }

    private IntField(string name, List<string> tokens) :
      base(name)
     {
      fieldValue = int.Parse(tokens[0], CultureInfo.InvariantCulture);
     }

    public static Field Create(string name, List<string> tokens)
     {
      return new IntField(name, tokens);
     }
