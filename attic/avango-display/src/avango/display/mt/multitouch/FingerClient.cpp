...
void FingerClient::setPositionHistorySize(unsigned int size)
{
    boost::mutex::scoped_lock lock(mutex);
    historysize = size;
}

void FingerClient::ReceiveThread() { socket->Run(); }

void FingerClient::ProcessPacket(const char* data, int size, const IpEndpointName& remoteEndpoint)
{
...
void FingerClient::ProcessBundle(const osc::ReceivedBundle& bundle, const IpEndpointName& remoteEndpoint)
{
    osc::int32 fseq = 0;
    int pattern = -1;
...
osc::int32 FingerClient::getUserIdForFinger(osc::int32 s_id)
{
...
void FingerClient::addUserIdToFinger(osc::int32 u_id, osc::int32 s_id)
{
...
void FingerClient::removeUserIdFromFinger(osc::int32 s_id)
{
    std::map<osc::int32, FingerInfo*>::iterator iter = cursor_id_map.find(s_id);
    if(iter != cursor_id_map.end())
    {
...
void FingerClient::ProcessKinectMessage(const osc::ReceivedMessage& message, const IpEndpointName& remoteEndpoint)
{
    osc::ReceivedMessageArgumentStream args = message.ArgumentStream();
...
void FingerClient::ProcessMessage(const osc::ReceivedMessage& message, const IpEndpointName& remoteEndpoint)
{
