...
#if 0
#define HOT_ENS_DEFAULT_PROTOCOL "Top:Heal:Switch:Leave:Inter:Intra:Elect:Merge:Sync:Suspect:Top_appl:Pt2pt:Frag:Stable:Mnak:Bottom"
#else
#define HOT_ENS_DEFAULT_PROTOCOL "BOTTOM:MNAK:PT2PT:CHK_FIFO:CHK_SYNC:TOP_APPL:STABLE:VSYNC:SYNC:ELECT:INTRA:INTER:LEAVE:SUSPECT:HEAL:TOP"
#endif
...
#define HOT_ENS_MAX_IO_NAME_LENGTH 128
     ...
     */
    typedef void (*hot_ens_ReceiveCast_cback)(hot_gctx_t gctx, void* env, hot_endpt_t* origin, hot_msg_t msg);

    /* Received a point-to-point message.
     */
    typedef void (*hot_ens_ReceiveSend_cback)(hot_gctx_t gctx, void* env, hot_endpt_t* origin, hot_msg_t msg);

    /* Accepted a new view.
     */
    typedef void (*hot_ens_AcceptedView_cback)(hot_gctx_t gctx, void* env, hot_view_state_t* view_state);

    /* Got a heartbeat event (invoked periodically).
     * Current time (in milliseconds) is specified.
     */
    typedef void (*hot_ens_Heartbeat_cback)(hot_gctx_t gctx, void* env, unsigned rate);

    /* The group is about to block for a view change
     */
    typedef void (*hot_ens_Block_cback)(hot_gctx_t gctx, void* env);

    /* The member has left the group.
     */
    typedef void (*hot_ens_Exit_cback)(hot_gctx_t gctx, void* env);
...
#define HOT_ENS_MAX_TRANSPORTS_LENGTH 128
     ...
     */
    hot_err_t hot_ens_Join(hot_ens_JoinOps_t* ops, hot_gctx_t* gctxp /*OUT*/
    );
     ...
     */
    hot_err_t hot_ens_Cast(hot_gctx_t g, hot_msg_t msg, hot_ens_MsgSendView* send_view /*OUT*/
    );
     ...
     * is returned in send_view, if send_view is not NULL.
     */
    hot_err_t hot_ens_Send(hot_gctx_t g, hot_endpt_t* dest, hot_msg_t msg, hot_ens_MsgSendView* send_view /*OUT*/
    );

    /* Report specified group members as failure-suspected.
     */
    hot_err_t hot_ens_Suspect(hot_gctx_t gctx, hot_endpt_t* suspects, int nsuspects);

    /* Inform Ensemble that the state-transfer is complete.
     */
    hot_err_t hot_ens_XferDone(hot_gctx_t gctx);

    /* Request a protocol change.
     */
    hot_err_t hot_ens_ChangeProtocol(hot_gctx_t gctx, char* protocol_name);

    /* Request a protocol change (specify properties).
     */
    hot_err_t hot_ens_ChangeProperties(hot_gctx_t gctx, char* properties);

    /* Request a new view to be installed.
     */
    hot_err_t hot_ens_RequestNewView(hot_gctx_t gctx);

    /* Request a rekey operation.
     */
    hot_err_t hot_ens_Rekey(hot_gctx_t gctx);

    hot_err_t hot_ens_MLPrintOverride(void (*handler)(char* msg));

    hot_err_t hot_ens_MLUncaughtException(void (*handler)(char* info));
