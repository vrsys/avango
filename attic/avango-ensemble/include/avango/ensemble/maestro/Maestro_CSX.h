...
{
    Maestro_CSX_XferEnv(Maestro_CSX* theCsx, int theID) : csx(theCsx), xid(theID) {}
...
class Maestro_CSX : public Maestro_ClSv
{
  public:
    ...
    MAESTRO_INLINE virtual void getState(Maestro_XferID& xferID, Maestro_Message& requestMsg, Maestro_Message& stateMsg, Maestro_XferStatus& xferStatus);

    MAESTRO_INLINE virtual void getState(Maestro_XferID& xferID, Maestro_Message& requestMsg, Maestro_XferStatus& xferStatus);

    MAESTRO_INLINE virtual void sendState(Maestro_EndpID& dest, Maestro_XferID& xferID, Maestro_Message& stateMsg);
  ...
  protected:
    ...
    virtual void csx_AcceptedView_Callback(Maestro_CSX_ViewData&, Maestro_Message&) {}

    // Message callbacks:

    virtual void csx_ReceiveCast_Callback(Maestro_EndpID&, Maestro_Message&) {}

    virtual void csx_ReceiveScast_Callback(Maestro_EndpID&, Maestro_Message&) {}

    virtual void csx_ReceiveSend_Callback(Maestro_EndpID&, Maestro_Message&) {}

    virtual void csx_ReceiveLsend_Callback(Maestro_EndpID&, Maestro_Message&) {}
    ...
    virtual void stateTransfer_Callback(Maestro_XferID& xferID) { xferDone(xferID); }
  ...
  private:
    ...
    MAESTRO_INLINE void clSv_AcceptedView_Callback(Maestro_ClSv_ViewData& viewData, Maestro_Message& viewMsg);

    // Message callbacks:

    void clSv_ReceiveCast_Callback(Maestro_EndpID& origin, Maestro_Message& msg) { csx_ReceiveCast_Callback(origin, msg); }

    void clSv_ReceiveScast_Callback(Maestro_EndpID& origin, Maestro_Message& msg) { csx_ReceiveScast_Callback(origin, msg); }

    void clSv_ReceiveSend_Callback(Maestro_EndpID& origin, Maestro_Message& msg) { csx_ReceiveSend_Callback(origin, msg); }

    void clSv_ReceiveLsend_Callback(Maestro_EndpID& origin, Maestro_Message& msg) { csx_ReceiveLsend_Callback(origin, msg); }
    ...
    void askState(Maestro_EndpID& server, Maestro_Message& msg) { Maestro_ClSv::askState(server, msg); }

    void sendState(Maestro_EndpID& dest, Maestro_Message& msg) { Maestro_ClSv::sendState(dest, msg); }
