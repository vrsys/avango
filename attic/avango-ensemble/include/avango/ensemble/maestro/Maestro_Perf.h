...
class Maestro_Perf
{
  public:
    ...
    void end(int point)
    {
        ...
        total_latency[point] += (after[point].tv_sec - before[point].tv_sec) * 1000000 + (after[point].tv_usec - before[point].tv_usec);
    }
    ...
    void print()
    {
        ...
        for(i = 0; i < PERFPTS; i++)
        {
            if(ncalls[i] > 0)
            {
                std::cout << i << ": " << descr[i] << " " << (total_latency[i] / ncalls[i]) << " usec (over " << ncalls[i] << " calls)" << std::endl;
            }
        }
        std::cout << "********************************************************" << std::endl;
        reset();
