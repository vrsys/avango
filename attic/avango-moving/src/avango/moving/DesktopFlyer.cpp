...
/* virtual */ void
av::moving::DesktopFlyer::fieldHasChanged(const av::Field& field)
{
  av::osg::MatrixTransform::fieldHasChanged(field);
}

/* virtual */ void
av::moving::DesktopFlyer::evaluate()
{
  ...
  if (move_z_active)
  {
    if (move_forward) move_z_input = 1;
    if (move_backward) move_z_input = -1;
    if (std::abs(move_z_input) < std::abs(mouse_y))
      move_z_input = mouse_y * ms;
  }
  if (move_x_active)
  {
    if (move_right) move_x_input = 1;
    if (move_left) move_x_input = -1;
    if (std::abs(move_x_input) < std::abs(mouse_x))
      move_x_input = mouse_x * ms;
  }
  if (move_y_active)
  {
    if (move_up) move_y_input = 1;
    if (move_down) move_y_input = -1;
    if (std::abs(move_y_input) < std::abs(mouse_y))
      move_y_input = mouse_y * ms;
  }
  if (rot_z_active)
  {
    if (rot_ccw) rot_z_input = 1;
    if (rot_cw) rot_z_input = -1;
    if (std::abs(rot_z_input) < std::abs(mouse_x))
      rot_z_input = mouse_x * ms;
  }
  if (rot_x_active)
  {
    if (rot_right) rot_x_input = 1;
    if (rot_left) rot_x_input = -1;
    if (std::abs(rot_x_input) < std::abs(mouse_x))
      rot_x_input = mouse_x * ms;
  }
  if (rot_y_active)
  {
    if (rot_up) rot_y_input = 1;
    if (rot_down) rot_y_input = -1;
    if (std::abs(rot_y_input) < std::abs(mouse_y))
      rot_y_input = mouse_y * ms;
  }

  bool mouse_triggered =
    (rot_xy_trigger || move_xz_trigger || move_xy_trigger || move_y_rot_z_trigger);
  ...
  if (move_acc>0)
  {
    current_speed_z = (!halt||move_z_active) * mOldSpeedZ + move_z_input *
    move_acc * mFrameTime;

    current_speed_x = (!halt||move_x_active) * mOldSpeedX + move_x_input *
    move_acc * mFrameTime;

    current_speed_y = (!halt||move_y_active) * mOldSpeedY + move_y_input *
    move_acc * mFrameTime;
  }
  else
  {
  ...
  {
    if ((std::abs(current_speed_x)>0)||(std::abs(current_speed_y)>0)||
        (std::abs(current_speed_z)>0))
    {
      ...
      current_speed_x = (move_x_input != 0 ? current_speed_x :
                         ((dir_x != dir_x_new) ? 0 : current_speed_x + brake_x));
      current_speed_y = (move_y_input != 0) ? current_speed_y : (dir_y != dir_y_new) ?
        0 : current_speed_y + brake_y;
      current_speed_z = (move_z_input != 0) ? current_speed_z : (dir_z != dir_z_new) ?
        0 : current_speed_z + brake_z;
    }
  }
  ...
  Matrix.setValue(
    ::osg::Matrix::translate(::osg::Vec3(mMoveX,mMoveY,-mMoveZ)) *
    ::osg::Matrix::rotate(
      mRotX, ::osg::Vec3(0.0,1.0,0.0),
      mRotY, ::osg::Vec3(1.0,0.0,0.0),
      mRotZ, ::osg::Vec3(0.0,0.0,1.0)
    )
    * Matrix.getValue()
  );
