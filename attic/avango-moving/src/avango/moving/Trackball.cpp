...
::osg::Vec3 projectToSphere(::osg::Vec2 position)
{
...
av::moving::Trackball::Trackball() : mLastDirection(0.0, 0.0), mLastProjected(0.0, 0.0, 0.0), mDragging(false)
{
...
void av::moving::Trackball::initClass()
{
...
/* virtual */ void av::moving::Trackball::fieldHasChanged(const av::Field& field)
{
...
/* virtual */ void av::moving::Trackball::evaluate()
{
    ...
    if(mDragging && newDragging)
    {
        ...
        {
            Matrix.setValue(mCenterTransInv * ::osg::Matrix::rotate(projected, mLastProjected) * CenterTransform.getValue() * Matrix.getValue());
        }
        else if(ZoomTrigger.getValue())
        {
            const ::osg::Vec2 offset = mLastDirection - Direction.getValue();
            Matrix.setValue(mCenterTransInv * ::osg::Matrix::translate(0.0, 0.0, -offset[1]) * CenterTransform.getValue() * Matrix.getValue());
        }
        else if(PanTrigger.getValue())
        {
            const ::osg::Vec2 offset = mLastDirection - Direction.getValue();
            Matrix.setValue(mCenterTransInv * ::osg::Matrix::translate(offset[0], offset[1], 0.0) * CenterTransform.getValue() * Matrix.getValue());
        }
    }
