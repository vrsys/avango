...
av::osg::viewer::GraphicsWindow::GraphicsWindow()
    : mStereoMode(STEREO_MODE_NONE), mDoubleBuffer(false), mShowCursor(false), mFullScreen(false), mSizeFieldsHasChanged(false), mNumStencilBits(0), mDragEvent(false), mMoveEvent(false),
...
/* static */ void av::osg::viewer::GraphicsWindow::initClass()
{
...
/* virtual */ void av::osg::viewer::GraphicsWindow::fieldHasChangedLocalSideEffect(const av::Field& field)
{
    if(&field == &WantedPositionX || &field == &WantedPositionY || &field == &WantedWidth || &field == &WantedHeight || &field == &Decoration) // windows decoration affects the window position
    {
...
/* virtual */ void av::osg::viewer::GraphicsWindow::evaluateLocalSideEffect()
{
    // delete window, if settings have changed which can't be updated dynamically
    if(mOsgGraphicsWindow.valid() && (mScreenIdentifier != ScreenIdentifier.getValue() || mStereoMode != StereoMode.getValue() || mDoubleBuffer != DoubleBuffer.getValue() ||
                                      mNumStencilBits != NumStencilBits.getValue() || mOsgSharedContextMaster.valid() != SharedContextMaster.getValue().isValid() ||
                                      mOsgSharedContextMaster.valid() && mOsgSharedContextMaster != SharedContextMaster.getValue()->getOsgGraphicsWindow()))
    {
    ...
    if(!mOsgGraphicsWindow.valid() && ::osg::GraphicsContext::getWindowingSystemInterface() != 0)
    {
        ...
        ::osg::GraphicsContext::getWindowingSystemInterface()->getScreenResolution(*traits, screenWidth, screenHeight);
        ...
        mOsgGraphicsWindow = dynamic_cast<::osgViewer::GraphicsWindow*>(::osg::GraphicsContext::createGraphicsContext(traits.get()));
    ...
    if(mOsgGraphicsWindow.valid())
    {
        if(mSizeFieldsHasChanged)
        {
            if(mFullScreen)
            {
                ...
                mOsgGraphicsWindow->setWindowRectangle(0, 0, ScreenWidth.getValue(), ScreenHeight.getValue());
            }
            else
            {
                mOsgGraphicsWindow->setWindowDecoration(Decoration.getValue());
                mOsgGraphicsWindow->setWindowRectangle(WantedPositionX.getValue(), WantedPositionY.getValue(), WantedWidth.getValue(), WantedHeight.getValue());
            }
        ...
        {
            const double height = RealScreenWidth.getValue() * ScreenHeight.getValue() / ScreenWidth.getValue();
...
/* virtual */ void av::osg::viewer::GraphicsWindow::evaluate()
{
            ...
            */
            ...
            if(!MouseFixed.getValue() || (eventPos[0] != width / 2) || (eventPos[1] != height / 2))
            {
                ...
                if(!MouseFixed.getValue())
                {
                    ...
                    if(width > 1 && height > 1)
                    {
                        mousePosNorm.set(-1.0 + 2.0 * mousePos[0] / (width - 1.0), 1.0 - 2.0 * mousePos[1] / (height - 1.0));
                    }
                ...
                if(width > 1 && height > 1)
                {
                    mouseMoveNorm.set(2.0 * mouseMove[0] / (width - 1.0), -2.0 * mouseMove[1] / (height - 1.0));
                }
                if(mouseMove.length2() < 0.1)
...
::osgViewer::GraphicsWindow* av::osg::viewer::GraphicsWindow::getOsgGraphicsWindow() const { return mOsgGraphicsWindow.get(); }

::osg::Vec2 av::osg::viewer::GraphicsWindow::getRealWindowSize()
{
...
av::osg::viewer::GraphicsWindow::CallbackHandle av::osg::viewer::GraphicsWindow::addWindowChangedCallback(const ChangedCallback& callback) { return mWindowChangedSignal.connect(callback); }

av::osg::viewer::GraphicsWindow::CallbackHandle av::osg::viewer::GraphicsWindow::addSizeChangedCallback(const ChangedCallback& callback) { return mSizeChangedSignal.connect(callback); }

/* static */ void av::osg::viewer::GraphicsWindow::removeCallback(const CallbackHandle& handle) { handle.disconnect(); }
