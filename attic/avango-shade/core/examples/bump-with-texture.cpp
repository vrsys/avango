...
{
    boost::shared_ptr<shade::shaders::Plastic> specular(new shade::shaders::Plastic(0.8, 0.2));
    ...
    boost::shared_ptr<shade::shaders::Texture2D> tex_pattern(new shade::shaders::Texture2D);
    tex_pattern->texture_unit.set(example::make_texture("examples/pattern.dds"));
    tex_pattern->uv = uvcoord;
    boost::shared_ptr<shade::shaders::Texture2D> tex_bump(new shade::shaders::Texture2D);
    tex_bump->texture_unit.set(example::make_texture("examples/dots.dds"));
    tex_bump->uv = uvcoord;
    specular->color = tex_pattern;
    specular->coordinate_system = coordinate_system;
    coordinate_system->normal_map = tex_bump;
...
{
    example::init(argc, argv, "SHADE Bump Mapping");
