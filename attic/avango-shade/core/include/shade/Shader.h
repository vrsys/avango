...
namespace shade
{
  ...
  >
  class ShaderBase :
    public detail::BasesShader<Base1, Base2, Base3, Base4>
  {
  public:
    ...
    template<class T> T invoke(const std::string& name);
    template<class T> T invoke_inline(const std::string& content);
    template<class T> T invoke_inline(boost::function<void (formatter::Generator&)> generator);
  ...
  protected:
    ...
    template<class A> static MethodAccessor def(A (Derived::*v)(void), std::string name, ShaderEnvironment env = invariant_shader);
    template<class A, class B> static MethodAccessor def(A (Derived::*v)(B), std::string name, ShaderEnvironment env = invariant_shader);
    template<class A, class B, class C> static MethodAccessor def(A (Derived::*v)(B, C), std::string name, ShaderEnvironment env = invariant_shader);
    template<class A, class B, class C, class D> static MethodAccessor def(A (Derived::*v)(B, C, D), std::string name, ShaderEnvironment env = invariant_shader);

    template<class T> static AttributeAccessor def(T Derived::* type, std::string name, ShaderEnvironment env = invariant_shader);
