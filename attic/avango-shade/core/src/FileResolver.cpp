...
void shade::FileResolver::append_path(const std::string& path)
{
  m_path.push_back(path);
}

void shade::FileResolver::prepend_path(const std::string& path)
{
  m_path.insert(m_path.begin(), path);
}
...
std::string shade::FileResolver::load(const std::string& name) const
{
  ...
  for (std::list<std::string>::const_iterator i = m_path.begin();
      i != m_path.end();
      ++i)
  {
...
namespace shade
{
  ...
  void append_env_path(FileResolver& resolver, const std::string& env_path)
  {
    ...
    std::for_each(token_line.begin(), token_line.end(),
        bind(&FileResolver::append_path, boost::ref(resolver), _1));
  }
