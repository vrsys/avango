...
shade::GLSLWrapper::ShaderType shade::GLSLWrapper::m_env_to_gl[shade::num_shader_environment] =
{
  GLSLWrapper::vertex_shader,
  GLSLWrapper::fragment_shader,
  GLSLWrapper::geometry_shader
};

shade::GLSLWrapper::GLSLWrapper(void) :
  m_gl_program(0),
  m_gl_shader(shade::num_shader_environment)
{
}
...
bool shade::GLSLWrapper::has_shader(ShaderEnvironment env) const
{
  return m_gl_shader[env];
}
...
std::string shade::GLSLWrapper::get_link_log(void) const
{
  return GetProgrammInfoLog(m_gl_program);
}

void shade::GLSLWrapper::make_current(void)
{
  UseProgram(m_gl_program);
}
...
shade::GLSLWrapper::Handle shade::GLSLWrapper::get_link_name(const std::string& name)
{
  return GetUniformLocation(m_gl_program, name.c_str());
}
...
std::string shade::GLSLWrapper::GetProgrammInfoLog(Handle program) const
{
  return std::string();
}
