...
shade::Program::Program(boost::shared_ptr<Shader> shader, boost::shared_ptr<GLSLWrapper> wrapper) :
  m_shader(shader),
  m_wrapper(wrapper),
  m_component(new Component(shader))
{
...
void shade::Program::append_search_path(const std::string& path)
{
  m_resolver.append_path(path);
}

void shade::Program::prepend_search_path(const std::string& path)
{
  m_resolver.prepend_path(path);
}
...
bool shade::Program::requires_compilation(void)
{
  return m_component->requires_compilation();
}

void shade::Program::upload(void)
{
  m_component->upload(m_wrapper);
}

bool shade::Program::requires_upload(void)
{
  return m_component->requires_upload();
}
