...
MarkBuffer::MarkBuffer(void) :
  m_string_cache_valid(false)
{
}

MarkBuffer::MarkBuffer(const std::string& content) :
  m_buffer(content),
  m_string_cache_valid(false)
{
}
...
MarkBuffer::iterator MarkBuffer::begin(void) const
{
...
MarkBuffer::iterator::iterator(void) :
  m_cur_file_mark(new FileMark)
{
}
...
MarkBuffer::iterator::iterator(const MarkBuffer* buffer, std::string::size_type pos) :
  ...
  m_position(pos),
  m_line_number(0),
  m_mark_iter(buffer->m_mark_list.begin()),
  m_cur_file_mark(new FileMark)
{
  update_mark_iter();
}

char MarkBuffer::iterator::operator*(void) const
{
  return m_buffer->str()[m_position];
}
...
bool MarkBuffer::iterator::operator==(const iterator& other) const
{
...
FileMark MarkBuffer::iterator::get_file_mark(void) const
{
  return FileMark(m_cur_file_mark->get_filename(), m_cur_file_mark->get_line_number() + m_line_number);
}
