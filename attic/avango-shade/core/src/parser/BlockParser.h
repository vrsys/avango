...
struct CallClosure : public boost::spirit::classic::closure<CallClosure, CallValues>
{
    ...
    void insert_parameter(boost::shared_ptr<Segment> param) { values().parameters.push_back(param); }
...
struct BlockClosure : public boost::spirit::classic::closure<BlockClosure, boost::shared_ptr<Segment>, std::vector<boost::shared_ptr<Segment>>>
{
    member1 body;
    member2 segments;
...
{
    segments.push_back(boost::shared_ptr<Segment>(new CallSegment(values.name, values.parameters.begin(), values.parameters.end(), values.begin, values.end)));
}
...
void push_segment(std::vector<boost::shared_ptr<Segment>>& segments, boost::shared_ptr<Segment> obj) { segments.push_back(obj); }
...
struct Block : public boost::spirit::classic::grammar<Block, BlockClosure::context_t>
{
    ...
    struct definition
    {
        ...
        definition(Block const& self)
        {
            ...
            foreach
                = (str_p("FOREACH") >> ch_p('(') >> identifier[bind(&ForeachValues::index)(foreach.values) = construct_<std::string>(arg1, arg2)] >> ch_p(',') >>
                   identifier[bind(&ForeachValues::list)(foreach.values) = construct_<std::string>(arg1, arg2)] >> ch_p(')') >> ch_p('{') >> self[bind(&ForeachValues::body)(foreach.values) = arg1] >>
                   ch_p('}'))[bind(&ForeachClosure::set_region)(var(foreach), arg1, arg2)];

            expression = (+(function[bind(&push_callinvocation)(expression.segments, arg1)] | identifier[bind(&push_identifier)(expression.segments, arg1, arg2)] |
                            ('(' >> expression_list[bind(&push_segment)(expression.segments, arg1)] >> ')') | (~chset<>(",;){}"))))[bind(&BlockClosure::build)(var(expression), arg1, arg2)];
            expression_list = (!(expression[bind(&push_segment)(expression_list.segments, arg1)]) >>
                               *(',' >> expression[bind(&push_segment)(expression_list.segments, arg1)]))[bind(&BlockClosure::build)(var(expression_list), arg1, arg2)];
            statement = *(str_p("if") | str_p("switch") | str_p("case") | str_p("for") >> '(' >> expression >> ';' >> expression >> ';' >> expression >> ')' |
                          foreach[bind(&push_foreach)(self.segments, arg1)] | expression[bind(&push_segment)(self.segments, arg1)] | chset<>(";,") | '{' >> statement >> '}');

            parameter = expression[bind(&CallClosure::insert_parameter)(var(function), arg1)];
            function = (identifier[bind(&CallValues::name)(function.values) = construct_<std::string>(arg1, arg2)] >> '(' >> !(parameter >> *(',' >> parameter)) >>
