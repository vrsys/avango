...
ForeachSegment::ForeachSegment(const std::string& index, const std::string& list, boost::shared_ptr<Segment> body, const formatter::MarkBuffer::iterator& begin, const formatter::MarkBuffer::iterator& end) :
  Segment(begin, end),
  m_index(index),
  m_list(list),
  m_body(body)
