...
void MethodInstantiator::add_method(boost::shared_ptr<MethodTemplate> method)
{
  m_methods[method->get_name()] = method;
}
...
std::string MethodInstantiator::call_method(const std::string& name,
    const ValueList& parameters,
    const ScopeLayer& scope,
    std::ostream& error_log) const
{
  ...
  else
  {
    ...
    instantiate_result = i->second->instantiate(unique_name, parameters, *generator, scope, error_log,
        boost::bind(&MethodInstantiator::call_method, this, _1, _2, _3, _4));
