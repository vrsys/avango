...
SHADE_CLASS_INIT(CameraSpace, "CameraSpace.glsl", 
    SHADE_NONE,
    SHADE_DEFS((position)(normal))
    )

shade::vec4<> CameraSpace::transform(vec4<>)
{
  return invoke< vec4<> >("CameraSpace_transform_impl");
}
...
shade::vec3<shade::Type> CameraSpace::get(void)
{
  return invoke_inline< vec3<> >(boost::bind(&CameraSpace::get_inline, boost::ref(*this), _1));
}

shade::vec3<> CameraSpace::get_transformed_normal(void)
{
  return invoke< vec3<> >("CameraSpace_get_transformed_normal_impl");
}

shade::vec3<> CameraSpace::get_transformed_eyepoint(void)
{
  return invoke< vec3<> >("CameraSpace_get_transformed_eyepoint_impl");
}

shade::void_<> CameraSpace::init_vertex(void)
{
  return invoke< void_<> >("CameraSpace_init_vertex_impl");
}
