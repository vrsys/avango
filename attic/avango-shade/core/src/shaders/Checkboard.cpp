...
SHADE_CLASS_INIT(Checkboard, "Checkboard.glsl", 
    SHADE_ENV_DEFS(shade::fragment_shader, (get_pattern)),
    SHADE_DEFS((position)(scale)(color1)(color2))
    )

shade::vec4<shade::Type> Checkboard::get_pattern(void)
{
  return invoke< vec4<Type> >("Checkboard_get_pattern_impl");
}
...
shade::vec4<> Checkboard::get(void)
{
  return invoke_inline< vec4<> >(boost::bind(&Checkboard::get_inline, boost::ref(*this), _1));
}
