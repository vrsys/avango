...
SUITE(Interpolator)
{
    ...
    {
        CompilationFixture(void) : interpolator(new shade::shaders::Interpolator<shade::vec2>), interpolator_id(interpolator->get_uniq_id()), component(interpolator), acc(resolver)
        {
    ...
    TEST_FIXTURE(CompilationFixture, Methods)
    {
        ...
        CHECK_EQUAL(shade::formatter::Constants::void_, fmt.get_inline_method(class_interpolator, initializer_name).first);
        CHECK(shade::formatter::check_dump_output("<statement>"
                                                  "<vardef>"
