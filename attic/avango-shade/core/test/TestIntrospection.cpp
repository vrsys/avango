...
SHADE_CLASS_INIT(Simple, "<undefined>", SHADE_NONE, SHADE_ENV_DEFS(shade::application_stage, (public_simple_value)) SHADE_DEFS((private_simple_value)))
...
SHADE_CLASS_INIT(Derived, "<undefined>", SHADE_NONE, SHADE_ENV_DEFS(shade::application_stage, (public_derived_value)) SHADE_DEFS((private_derived_value)))

struct StoreNames
{
    void operator()(shade::Type* type, const std::string& name) { attributes[name] = type; }
...
SUITE(Introspection)
{
    ...
    {
        ObjRefFixture(void) : accessor(dynamic_cast<shade::types::ObjrefAccessor*>(&value)) {}

        bool is_accessor_valid(void) const { return (accessor != 0); }
    ...
    struct ListFixture
    {
        ...
        ListFixture(void) : accessor(dynamic_cast<shade::types::ListAccessor*>(&derived_list)), value1(new Derived), value2(new Derived) {}
