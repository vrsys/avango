...
SUITE(Varying)
{
    class ClassWithVarying : public shade::ShaderBase<ClassWithVarying>
    {
      public:
        ...
        ClassWithVarying(void) { shade::UVWInstance::init(); }
    ...
    struct CompilationFixture
    {
        CompilationFixture(void)
            : value(new ClassWithVarying), value_id(value->get_uniq_id()), component(value), acc(resolver), fmt_vertex(shade::vertex_shader), fmt_geometry(shade::geometry_shader),
