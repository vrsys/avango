...
namespace
{
  ...
  {
    VertexShaderFixture(void) :
      generator(shade::vertex_shader)
    {
    }
...
SUITE(TestGL2Generator)
{
  ...
  TEST_FIXTURE(VertexShaderFixture, TestListLiteral)
  {
    ...
    CHECK_EQUAL("LIST("
        +boost::lexical_cast<std::string>(begin)
        +", "
  ...
  void generate_function_definition(shade::formatter::GL2Generator& generator, int parameters, bool declaration = false,
      shade::formatter::Constants::Qualifier qualifier = shade::formatter::Constants::none)
  {
