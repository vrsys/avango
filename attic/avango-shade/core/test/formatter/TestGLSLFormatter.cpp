...
SUITE(TestGLSLFormatter)
{
  TEST(WriteAttributes)
  {
    ...
    fmt.insert_attribute(shade::formatter::Constants::sampler2D, "obj_0x1", "tex",
        shade::formatter::Constants::uniform);
    fmt.insert_attribute(shade::formatter::Constants::float_, "obj_0x2", "luminance",
        shade::formatter::Constants::output);

    fmt.generate(result_generator);
    CHECK(shade::formatter::check_partial_dump_output(
      "<statement>"
        "<vardef>"
  ...
  TEST(WriteAttributesWithInit)
  {
    ...
    generator = fmt.begin_insert_init_attribute(shade::formatter::Constants::vec3,
        "obj_0x3", "position",
        shade::formatter::Constants::const_);
    generator->handle_literal_vec3(1.1, 2.2, 3.3);
    fmt.end_insert_init_attribute();

    generator = fmt.begin_insert_init_attribute(shade::formatter::Constants::list,
        "obj_0x1", "lights");
  ...
  struct EmptyDispatcherFixture
  {
    ...
    EmptyDispatcherFixture(void) :
      fmt(shade::vertex_shader, result_generator)
    {
  ...
  TEST_FIXTURE(EmptyDispatcherFixture, WriteEmptyDispatcherPre)
  {
    CHECK(shade::formatter::check_partial_dump_output(
      get_expected_header()+
      "</funcdef>"
  ...
  std::string output_expected_simple_dispatcher(shade::ShaderEnvironment env)
  {
    ...
    hout <<
                    "<func_call_param><id>obj_0x2_intensity</id></func_call_param>"
  ...
  TEST(WriteDispatcher)
  {
    CHECK(shade::formatter::check_partial_dump_output(
          output_expected_simple_dispatcher(shade::vertex_shader),
          output_simple_dispatcher(shade::vertex_shader)));
  }

  TEST(WriteInlineDispatcher)
  {
    CHECK(shade::formatter::check_partial_dump_output(
          output_expected_simple_dispatcher(shade::vertex_shader),
          output_simple_dispatcher(shade::vertex_shader, true)));
  }

  TEST(WriteDispatcherInGeometryShader)
  {
    CHECK(shade::formatter::check_partial_dump_output(
          output_expected_simple_dispatcher(shade::geometry_shader),
          output_simple_dispatcher(shade::geometry_shader)));
  }
  ...
  void generate_property_begin(shade::formatter::Generator& generator, shade::formatter::Constants::Type retval,
      const std::string& method, bool setter = false, bool declaration = false)
  {
  ...
  void generate_switch_case_return_deferred_value(shade::formatter::Generator& generator, shade::ObjectMap::Index value,
      const std::string& func, shade::ObjectMap::Index obj, bool geometry = false)
  {
    generator.handle_switch_case_begin();
    generator.handle_literal_objref(value);
    generator.handle_switch_case_end();
  ...
  void generate_switch_case_assign_deferred_value(shade::formatter::Generator& generator, shade::ObjectMap::Index value,
      const std::string& func, shade::ObjectMap::Index obj, bool geometry = false)
  {
    generator.handle_switch_case_begin();
    generator.handle_literal_objref(value);
  ...
  TEST(WritePropertyDispatcher)
  {
    ...
    CHECK(shade::formatter::check_partial_dump_output(
          ...
          result));
    CHECK(shade::formatter::check_partial_dump_output(
          expected_pre_generator.get_code(),
          result));
  }
