...
{
  public:
    SegmentHandler(Scope& scope) : scope_(scope) {}

    void operator()(const std::string& name, ParameterIterator begin, ParameterIterator end, const std::string& return_type, const std::string& parameters, boost::shared_ptr<Segment> segment)
    {
        name_ = name;
    ...
    std::string get_content(void) const { return get_content(segment_.size() - 1); }
    ...
    std::string get_content(int index) const
    {
        ...
        segment_[index]->get_content(generator, scope_, boost::bind(&SegmentHandler::mangle_name, this, _1, _2, _3, _4), error_log);
        return generator.get_code();
    }

    int get_num_functions(void) const { return segment_.size(); }

    std::string get_function_name(void) const { return name_; }

    const std::vector<std::string>& get_parameters(void) const { return parameters_; }

    std::string get_return_string(void) const { return return_string_; }

    std::string get_parameters_string(void) const { return parameters_string_; }
...
SUITE(TestParser)
{
    struct Fixture
    {
        Fixture(void) : scope(global), handler(scope)
        {
        ...
        {
            return shade::formatter::check_dump_output("<funcdef>"
    ...
    {
        CHECK(parse("  void test() {\n"
                    "  Test();  "
                    "}\n",
                    generator,
                    translator));

        CHECK(check_void_function_block("<func_call>"
    ...
    TEST_FIXTURE(Fixture, DontParse) { CHECK(!parse("  vec3(0., 0., 0.);  ", generator, translator)); }
    ...
    TEST_FIXTURE(Fixture, FunctionInBlock)
    {
        ...
        CHECK(check_partial_function_block("<func_call>"
                                           "<func_call_name><id>Test</id></func_call_name>"
                                           "<func_call_param_list>"
                                           "<func_call_param><id>abc</id></func_call_param>"
    ...
    {
        CHECK(parse("void test(int value) {\n"
                    "}\n",
                    generator,
                    translator));

        CHECK(shade::formatter::check_dump_output("<funcdef>"
                                                  "<funcdef_type><verbatim>void</verbatim></funcdef_type>"
                                                  "<funcdef_name><id>test</id></funcdef_name>"
                                                  "<funcdef_param_list>"
    ...
    {
        CHECK(parse("void test(int value, double number) {\n"
                    "}\n",
                    generator,
                    translator));
    ...
    {
        CHECK(parse("void test(varying int value, invariant centroid varying double number, const sampler2D tex) {\n"
                    "}\n",
                    generator,
                    translator));
    ...
    {
        CHECK(parse("void test(int value[7], out double number[124]) {\n"
                    "}\n",
                    generator,
                    translator));
    ...
    TEST_FIXTURE(Fixture, VerfiySelfParameter)
    {
        ...
        CHECK(parse("void test(Class_SELF, int value) {\n"
                    "}\n",
                    generator,
                    translator));
    ...
    {
        CHECK(parse("void test() /* A comment */ {\n"
                    "  Test();\n"
                    "  // Ignore Test();\n"
                    "  Test();\n"
                    "}\n",
    ...
    TEST_FIXTURE(Fixture, ForeachLoop)
    {
        ...
        CHECK(check_void_function_block("<for>"
                                        "<for_init>"
                                        "<vardef>"
    ...
    TEST_FIXTURE(Fixture, NormalForLoop)
    {
        ...
        CHECK(check_void_function_block("<verbatim>"
                                        "for(int i = 1; i != 2; ++i)\n"
                                        "{\n"
