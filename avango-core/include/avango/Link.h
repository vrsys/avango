...
class Link : public AnyLink
{
  ...
  public:
     ...
     */
    Link() : AnyLink(0, T::getClassTypeId()) {}

    /**
     * Copy construct a new link.
     */
    Link(const Link<T>& link) : AnyLink(link.getBasePtr(), T::getClassTypeId()) {}

    /**
     * Copy construct a new link from a compatible type.
     */
    template <class Y>
    Link(const Link<Y>& link) : AnyLink(convertToBase(link.getPtr()), T::getClassTypeId())
    {
    }
     ...
     */
    Link(T* ptr) : AnyLink(ptr, T::getClassTypeId()) {}
     ...
     * \c null is returned.
     */
    T* getPtr() const { return static_cast<T*>(mBasePtr); }
     ...
     */
    static const Link<T>& empty() { return sEmptyLink; }
 ...
 */
template <typename T>
bool operator==(const Link<T>& lhs, const Link<T>& rhs)
{
 ...
 */
template <typename T>
bool operator!=(const Link<T>& a, const Link<T>& b)
{
    return !(a == b);
}
 ...
 */
template <typename T>
T* get_pointer(const Link<T>& link)
{
    return link.getPtr();
}
