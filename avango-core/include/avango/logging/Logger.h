...
class Logger
{
  public:
    ...
    {
        bool operator()(const boost::shared_ptr<Appender> p1, const boost::shared_ptr<Appender> p2) const { return p1.get() < p2.get(); }
    };
...
template <typename T>
inline Logger::Stream& Logger::Stream::operator<<(const T& t)
{
 ...
 * \ingroup av_logging
 */
AV_DLL inline Logger& getLogger(const std::string& name) { return logging::LoggerManagerInstance::get().getLogger(name); }

/**
 * Convenience definition, shorthand for Logger::getRootLogger.
 * \ingroup av_logging
 */
AV_DLL inline Logger& getRootLogger() { return logging::LoggerManagerInstance::get().getRootLogger(); }

} // namespace av

#ifdef TRACE_LOGGING
#define LOG_TRACE(trace_logger) (trace_logger).trace()
#else
