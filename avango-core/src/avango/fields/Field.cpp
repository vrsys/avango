...
{
  public:
    SetBoolFlag(bool& value) : mValueRef(value) { mValueRef = true; }
    ~SetBoolFlag() { mValueRef = false; }
...
void av::Field::setContainer(av::FieldContainer* c, unsigned int i, bool owned)
{
    AV_ASSERT(c);
...
av::FieldContainer* av::Field::getContainer() const
{
...
bool av::Field::isBound() const { return (mContainer != 0); }

unsigned int av::Field::getIndex() const { return mIndex; }

av::Field* av::Field::getConnectedField(unsigned int index)
{
...
unsigned int av::Field::getNumberOfConnectedFields() const { return mConnectedFrom.size(); }

void av::Field::enableDependency(bool on) { mFlags.dependencyEnabled = on; }

bool av::Field::dependencyEnabled() { return mFlags.dependencyEnabled; }

void av::Field::enableNotify(bool on) { mFlags.notifyEnabled = on; }

bool av::Field::notifyEnabled() { return mFlags.notifyEnabled; }

void av::Field::notify(Field* triggered_from)
{
#if defined(AVANGO_DEBUG)
    if(mFlags.hasNotified && !mAuditors.empty())
    {
        SingleField<std::string>* container_name = dynamic_cast<SingleField<std::string>*>(getContainer()->getField("Name"));

        AVANGO_LOG(logger,
                   logging::TRACE,
                   boost::str(boost::format("notify: "
                                            "%1% . %2% of type <%3%.%4%> %5%-pushed, %6% auditor%7% was already notified.") %
                              (0 != container_name ? container_name->getValue() : "unnamed") % getName() % getContainer()->getTypeId().getName() % getTypeId().getName() %
                              (mFlags.multiPush ? "multi" : "single") % mAuditors.size() % (1 != mAuditors.size() ? "s" : "")));
    }
#endif
...
void av::Field::touch() { fieldChanged(false); }
...
void av::Field::dontWrite(bool on) { mFlags.dontWrite = on; }

bool av::Field::isWritable() const { return !mFlags.dontWrite; }

void av::Field::dontDistribute(bool on) { mFlags.dontDistribute = on; }

void av::Field::needsDistribution(bool on) { mFlags.needsDistribution = on; }

bool av::Field::needsDistribution() const { return mFlags.needsDistribution; }

bool av::Field::isDistributable() const { return !mFlags.dontDistribute; }

bool av::Field::isMultiPush() const { return mFlags.multiPush; }

void av::Field::setMultiPush(bool onoff)
{
    if(!onoff)
    {
...
bool av::Field::isMultiInput() const { return mFlags.multiInput; }

void av::Field::setMultiInput(bool onoff)
{
    if(!onoff)
    {
...
bool av::Field::isOwned() const { return mFlags.owned; }

av::Field::Field() : mContainer(0), mIndex(0), mIsNotifying(false), mSrc(internal)
{
    mFlags.touched = false;
...
const std::string& av::Field::getName() const
{
...
void av::Field::fieldChanged(bool fromNet, Field* triggeredFrom)
{
    if(!mContainer)
    {
        return;
...
void av::Field::connectFrom(av::Field* field, bool dependent)
{
    if(!field)
    {
...
void av::Field::disconnect()
{
    ...
    for(InputFieldsList::iterator list_iter = mConnectedFrom.begin(); list_iter != mConnectedFrom.end(); ++list_iter)
    {
...
class first_equal_to
{
    using Pair = std::pair<A, B>;
    A mValue;

  public:
    first_equal_to(const A& value) : mValue(value) {}

    bool operator()(const Pair& x) { return x.first == mValue; }
};
} // namespace

void av::Field::disconnectFrom(av::Field* field)
{
    if(mConnectedFrom.empty())
    {
        return;
    ...
    InputFieldsList::iterator field_iter = std::find_if(mConnectedFrom.begin(), mConnectedFrom.end(), first_equal_to<Field*, bool>(field));
...
const av::Field::FieldPtrSet& av::Field::getAuditors() const { return mAuditors; }

void av::Field::disconnectAuditors()
{
...
void av::Field::addAuditor(av::Field* field)
{
    AV_ASSERT(field);
...
void av::Field::removeAuditor(av::Field* field)
{
    AV_ASSERT(field);
...
av::Field::FieldChangeSource av::Field::getFieldChangeSource() const { return mSrc; }

void av::Field::setFieldChangeSource(FieldChangeSource src) { mSrc = src; }

void av::Field::bind(av::FieldContainer* container, const std::string& name, bool owned)
{
    AV_ASSERT(container);

    if(isBound())
    {
        AVANGO_LOG(logger, logging::WARN, boost::str(boost::format("bind: %1%.%2% already bound") % (container ? container->getTypeId().getName() : "null-pointer") % name));
        return;
    }
...
av::Type av::Field::registerType(const std::string& classname, const std::string& parentname, av::Create* creator)
{
#if defined(AVANGO_DEBUG)
    AVANGO_LOG(logger, logging::DEBUG, boost::str(boost::format("registerType: registration request for type '%1%' with parent type '%2%'.") % classname % parentname));
#endif
    ...
    {
        AVANGO_LOG(logger, logging::WARN, boost::str(boost::format("registerType: type '%1%' already registered, returning '%2%'!") % classname % type.getName()));
        return type;
    }
    ...
    if(!parentname.empty())
    {
        ...
        {
            AVANGO_LOG(
                logger, logging::WARN, boost::str(boost::format("registerType: parent type '%1%' of type '%2%' doesn't exist, returning '%3%'!") % parentname % classname % Type::badType().getName()));
            return Type::badType();
        }
    }
    else
    {
        AVANGO_LOG(logger, logging::WARN, boost::str(boost::format("registerType: setting parent type '%1%' of type '%2%' to '%3%'!") % parentname % classname % Type::badType().getName()));
        parent = Type::badType();
    }
...
#if defined(AVANGO_DEBUG)
    AVANGO_LOG(logger, logging::DEBUG, boost::str(boost::format("registerType: created type '%1%' (id: %2%, parent: '%3%').") % type.getName() % type.getId() % type.getParent().getName()));
#endif
    return type;
}

av::InputStream& av::operator>>(av::InputStream& is, av::Field* field)
{
    field->read(is);
    return is;
}

av::OutputStream& av::operator<<(av::OutputStream& os, const av::Field* field)
{
    const_cast<av::Field*>(field)->write(os);
    return os;
}

void av::Field::writeConnection(av::OutputStream& os)
{
...
void av::Field::readConnection(av::InputStream& is)
{
    char con_flag;
    ...
    {
        AVANGO_LOG(logger, logging::WARN, boost::str(boost::format("read: can't resolve field connection for type '%1%'!") % (from_base ? from_base->getTypeId().getName() : "unnamed at 0x0")));
    }
}

void av::Field::evaluateDependencies(void)
{
    ...
    for(Field::InputFieldsList::iterator from_field_it = mConnectedFrom.begin(); from_field_it != mConnectedFrom.end(); ++from_field_it)
    {
