...
av::NetNode::NetNode() : mMember(0), mNumMembers(0), mWellKnownObjects(), mBlocked(false) {}
...
void av::NetNode::join(const std::string& groupName)
{
    ...
    logger.debug() << "av::NetNode::NetNode: "
                   << "joined group '" << groupName.c_str() << "', my eid: " << netEID();
#endif

    mIdCounter = 0;

#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::join: was called from: "
                   << "  pid: " << getpid();
...
av::NetID av::NetNode::generateUniqueId()
{
    assert(onAir());
#ifdef ZMQ_DISTRIBUTION_SUPPORT
    if(gServer)
...
const std::string& av::NetNode::netEID() const { return mEID; }

// handle network callbacks.
// this gives the network process time to things
void av::NetNode::handleNetworkSends()
{
...
void av::NetNode::handleNetworkReceives()
{
#ifdef ZMQ_DISTRIBUTION_SUPPORT
    if(gClient)
    {
...
void av::NetNode::queueStateFragment(const std::string& fragment, av::Msg& stateMsg) { mStateUpdates.push_back(std::pair<std::string, Msg>(fragment, stateMsg)); }

void av::NetNode::queueRemoveFragment(const std::string& fragment) { mStateRemoves.push_back(fragment); }

void av::NetNode::applyStateFragments()
{
#ifdef AVANGO_DEBUG
...
void av::NetNode::registerWellKnown(const Link<Distributed>& obj, const NetID& id)
{
#ifdef AVANGO_DEBUG
...
void av::NetNode::unregisterWellKnown(const Link<Distributed>& obj)
{
    // make sure we have already joined a group
    AV_ASSERT(onAir());
...
void av::NetNode::registerObj(const Link<Distributed>& obj) { registerObj(obj, generateUniqueId()); }

// if an object is created by someone else we use this version
void av::NetNode::registerObj(const Link<Distributed>& obj, const NetID& id)
{
#ifdef AVANGO_DEBUG
...
void av::NetNode::unregisterObj(const Link<Distributed>& obj)
{
    // make sure we have already joined a group
    AV_ASSERT(onAir());
...
void av::NetNode::notifyCreations()
{
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::notify_creations: "
                   << "creations pending: " << mPendingCreations.size();
#endif
...
void av::NetNode::notifyUpdates()
{
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::notify_updates: "
                   << "updates pending: " << mPendingUpdates.size();
#endif
...
void av::NetNode::notifyDeletes()
{
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::notify_deletes: "
                   << "deletes pending: " << mPendingDeletes.size();
#endif
...
bool av::NetNode::lookup(const NetID& id, Link<Distributed>& result)
{
    ...
    if(id.isWellKnown())
    {
#ifdef AVANGO_DEBUG
        {
            logger.debug() << "av::NetNode::lookup: "
                           << "searching w/in " << mWellKnownObjects.size() << " well-known objects";

            NetIDDstMap::iterator i = mWellKnownObjects.begin();

            while(i != mWellKnownObjects.end())
            {
                logger.debug() << "av::NetNode::lookup: " << (*i).first << ":" << (*i).second->netID();
                ++i;
            }
        }
#endif
...
void av::NetNode::joined(const std::string& myeid)
{
#ifdef AVANGO_DEBUG
...
void av::NetNode::setStateFragment(const std::string& fragment, av::Msg& stateMsg)
{
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::set_state_fragment: from " << fragment << " size: " << stateMsg.getSize() << " bytes.";
#endif
...
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::set_state_fragment: create messages: " << num_objects;
#endif
...
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::set_state_fragment: update messages: " << num_objects;
#endif
...
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::set_state_fragment: setting state fragment for " << fragment;
#endif
...
void av::NetNode::getStateFragment(const std::string& fragment, av::Msg& stateOutMsg)
{
...
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::get_state_fragment: for " << fragment << " size: " << state_msg.getSize() << " bytes.";
...
void av::NetNode::removeStateFragment(const std::string& fragment)
{
    if(mObjectMap.slotExists(fragment))
    {
...
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::remove_state_fragment: for " << fragment;
#endif
}

void av::NetNode::receiveMessage(const std::string& origin, av::Msg& msg)
{
...
void av::NetNode::consumeReceivedMessages()
{
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::consume_received_messages: " << mReceivedMessages.size() << " messages";
#endif
...
void av::NetNode::acceptNewView(const std::vector<std::string>& members, const std::vector<std::string>& newMembers, const std::vector<std::string>& departedMembers, av::Msg& msg)
{
    // set our own eid
...
void av::NetNode::distributeObject(av::Link<av::Distributed> obj)
{
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::distribute_object: '" << getName(obj) << "' w/ type id " << obj->getTypeId().getName().c_str();
#endif
...
void av::NetNode::undistributeObject(av::Link<av::Distributed> obj)
{
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::undistribute_object: '" << getName(obj) << "' w/ type id " << obj->getTypeId().getName().c_str();
#endif
...
// make an existing object distributed, make it local again, update it's state
void av::NetNode::distributeFieldContainer(av::Link<av::FieldContainer> obj) { distributeObject(Link<Distributed>(obj)); }

void av::NetNode::undistributeFieldContainer(av::Link<FieldContainer> obj) { undistributeObject(Link<Distributed>(obj)); }

void av::NetNode::updateDistributedObject(const av::Link<av::Distributed>& obj)
{
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::update_distributed_object: '" << getName(obj) << "' w/ type id " << obj->getTypeId().getName().c_str();
#endif
...
void av::NetNode::makeCreateMessage(av::Msg& msg, const av::Link<av::Distributed>& obj)
{
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::make_create_message: start: '" << obj->getTypeId().getName().c_str() << "' w/ id: " << obj->netID();
#endif
...
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::make_create_message: done: '" << obj->getTypeId().getName().c_str() << "' w/ id: " << obj->netID();
#endif
}

void av::NetNode::makeUpdateMessage(av::Msg& msg, const av::Link<av::Distributed>& obj)
{
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::make_update_message: start: '" << obj->getTypeId().getName().c_str() << "' w/ id: " << obj->netID();
#endif
...
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::make_update_message: done: '" << obj->getTypeId().getName().c_str() << "' w/ id: " << obj->netID();
#endif
}

void av::NetNode::makeDeleteMessage(av::Msg& msg, const av::Link<av::Distributed>& obj)
{
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::make_delete_message: start: '" << obj->getTypeId().getName().c_str() << "' w/ id: " << obj->netID();
#endif
...
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::make_delete_message: done: '" << obj->getTypeId().getName().c_str() << "' (netID deleted)";
#endif
}

// take them from the net and do the right thing
void av::NetNode::consumeMessage(av::Msg& msg)
{
    ...
    {
    case sCreateMsg:
        consumeCreateMessage(msg);
        break;
    case sUpdateMsg:
        consumeUpdateMessage(msg);
        break;
    case sDeleteMsg:
        consumeDeleteMessage(msg);
        break;
    case sPackedMsg:
        consumePackedMessage(msg);
        break;
...
void av::NetNode::consumeCreateMessage(av::Msg& msg)
{
    Type obj_type;
    NetID obj_id;
    ...
    {
        std::cout << "av::NetNode::consume_create_message: bad (" << obj_type.getName().c_str() << ")" << std::endl;
    }
...
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::consume_create_message: creating a '" << obj_type.getName().c_str() << "'";
#endif
...
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::consume_create_message: '" << obj_type.getName().c_str() << "' w/ id: " << obj_id;
#endif
}

void av::NetNode::consumeUpdateMessage(av::Msg& msg)
{
    Type obj_type;
    NetID obj_id;
...
#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::consume_update_message: '" << obj_type.getName().c_str() << "' w/ id: " << obj_id;
#endif
    ...
    if(!lookup(obj_id, obj))
    {
        ...
        std::cerr << "av::NetNode::consume_update_message: created dummy instance of type '" << obj_type.getName().c_str() << "' w/ id: " << obj_id << " for non-existent object" << std::endl;
    }
...
void av::NetNode::consumeDeleteMessage(av::Msg& msg)
{
    NetID obj_id;
    ...
    if(!found)
    {
        logger.warn() << "av::NetNode::consume_delete_message: "
                      << "object not found: " << obj_id;
        return;
    }

#ifdef AVANGO_DEBUG
    logger.debug() << "av::NetNode::consume_delete_message: " << obj_id;
#endif

    unregisterObj(obj);
}

void av::NetNode::consumePackedMessage(av::Msg& msg)
{
    int msg_count;
...
/* static */ std::map<std::string, std::string, std::less<std::string>> av::NetNode::sEnsOptions;
/* static */ char** av::NetNode::sEnsOptionsArgv = 0;

/* static */ void av::NetNode::setEnsOption(const std::string& option, const std::string& value) { sEnsOptions[option] = value; }

/* static */ bool av::NetNode::getEnsOption(const std::string& option, std::string& value)
{
    std::map<std::string, std::string, std::less<std::string>>::const_iterator i = sEnsOptions.find(option);
...
/* static */ char** av::NetNode::getEnsOptionsArgv()
{
