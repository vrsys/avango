...
av::FieldContainer::FieldContainer() : mEvaluateId(0u)
{
...
/* static */ void av::FieldContainer::initClass()
{
...
unsigned int av::FieldContainer::addField(Field* field, const std::string& fieldName)
{
    AV_ASSERT(field);
...
av::FieldContainer::FieldInfo* av::FieldContainer::getFieldInfo(const std::string& name)
{
...
av::FieldContainer::FieldInfo* av::FieldContainer::getFieldInfo(unsigned int index) { return (index < mFields.size() ? &mFields[index] : 0); }

unsigned int av::FieldContainer::getNumFields() { return mFields.size(); }

/* virtual */ void av::FieldContainer::addDynamicField(const std::string& typeName, const std::string& fieldName)
{
...
/* virtual */ void av::FieldContainer::addDynamicField(Field* field, const std::string& fieldName)
{
    // check if a field with the given name already exists
    FieldsIndex::iterator fieldIter = mFieldsIndex.find(fieldName);
    AV_ASSERT(fieldIter == mFieldsIndex.end());
...
/* virtual */
void av::FieldContainer::removeField(unsigned int index)
{
...
/* virtual */
void av::FieldContainer::removeDynamicField(const std::string& fieldName)
{
    // get the field id
    FieldsIndex::iterator fieldIter = mFieldsIndex.find(fieldName);
    AV_ASSERT(fieldIter != mFieldsIndex.end());
...
/* virtual */ bool av::FieldContainer::hasField(const std::string& name)
{
    FieldsIndex::const_iterator iter = mFieldsIndex.find(name);
    return (iter != mFieldsIndex.end());
}

av::Field* av::FieldContainer::getField(unsigned int index) { return (index < mFields.size() ? mFields[index].mField : 0); }

av::Field* av::FieldContainer::getField(const std::string& name)
{
    FieldInfo* field_info = getFieldInfo(name);
    return (field_info != 0 ? field_info->mField : 0);
}

const std::string& av::FieldContainer::getFieldName(unsigned int index) { return (index < mFields.size() ? mFields[index].mName : empty_string); }

av::FieldContainer::IDType av::FieldContainer::getId() const { return mId; }

bool av::FieldContainer::needsEvaluation() const { return mFlags.mNeedsEvaluation; }

void av::FieldContainer::callEvaluate()
{
...
double av::FieldContainer::lastChange() const { return mLastChange; }

void av::FieldContainer::scheduleForEvaluation()
{
...
void av::FieldContainer::touch(bool from_net)
{
    if(!from_net)
    {
...
void av::FieldContainer::enableNotify(bool on) { std::for_each(getFields().begin(), getFields().end(), std::bind2nd(std::mem_fun(&Field::enableNotify), on)); }

void av::FieldContainer::allowScheduling(bool on) { mFlags.mAllowScheduling = on; }

void av::FieldContainer::alwaysEvaluate(bool on)
{
...
void av::FieldContainer::evaluateDependency(FieldContainer& dependency)
{
    if(mFlags.mIsEvaluating)
...
/* virtual */ void av::FieldContainer::getNameCB(const av::SFString::GetValueEvent& event) { *(event.getValuePtr()) = ContainerPool::getNameByInstance(this); }

/* virtual */ void av::FieldContainer::setNameCB(const av::SFString::SetValueEvent& event) { ContainerPool::setNameForInstance(this, event.getValue()); }

/* static */ unsigned int av::FieldContainer::getNumberOfContainersToEvaluate() { return evaluation_list.size(); }

/* static */ void av::FieldContainer::evaluateAllContainers()
{
...
/* static */ void av::FieldContainer::unscheduleEvaluationForAllContainers() { evaluation_list.clear(); }
...
void av::FieldContainer::disconnectAllFields()
{
    FieldPtrVec& fields = getFields();
    for(FieldPtrVec::iterator eachField = fields.begin(); eachField != fields.end(); ++eachField)
    {
        (*eachField)->disconnect();
    }
}

std::vector<av::Field*>& av::FieldContainer::getFields()
{
    if(!mFlags.mFieldsCalculated)
    {
        FieldPtrVec emptyVec(mFields.size());
        mFieldPtrs.swap(emptyVec);
        std::transform(mFields.begin(), mFields.end(), mFieldPtrs.begin(), std::bind(&FieldInfo::mField, std::placeholders::_1));
        mFlags.mFieldsCalculated = true;
    }
    return mFieldPtrs;
}

bool av::FieldContainer::getFieldRef(const std::string& name, av::FieldRef& fr)
{
...
void av::FieldContainer::evaluate() { AVANGO_LOG(logger, logging::TRACE, "evaluated: called"); }

void av::FieldContainer::evaluateLocalSideEffect() { AVANGO_LOG(logger, logging::TRACE, "evaluteLocalSideEffect: called"); }

void av::FieldContainer::fieldHasChanged(const Field& /*field*/) { AVANGO_LOG(logger, logging::TRACE, "fieldHasChanged: called"); }

void av::FieldContainer::fieldHasChangedLocalSideEffect(const Field& /*field*/) { AVANGO_LOG(logger, logging::TRACE, "fieldHasChangedLocalSideEffect: called"); }

void av::FieldContainer::fieldChanged(const Field& field, bool from_net)
{
    ...
    AVANGO_LOG(
        logger,
        logging::DEBUG,
        boost::str(boost::format("FieldContainer::fieldChanged: '%1%' @0x%2%: field '%3%' changed %4%.") % getTypeId().getName() % this % field.getName() % (from_net ? "from net" : "locally")));
...
void av::FieldContainer::read(InputStream& is)
{
    FieldPtrVec& fields = getFields();
...
#if defined(AVANGO_DISTRIBUTION_SUPPORT)
/* virtual */ void av::FieldContainer::push(av::Msg& msg)
{
    ...
    for(i = fields.begin(); i != fields.end(); ++i)
    {
        ...
        if(msg.getType() == Msg::relative)
        {
            ...
            if(field->isDistributable() && field->mFlags.needsDistribution)
            {
...
/* virtual */ void av::FieldContainer::pop(av::Msg& msg)
{
...
void av::FieldContainer::write(OutputStream& os)
{
    FieldPtrVec& fields = getFields();
