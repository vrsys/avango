...
av::InputStream::InputStream() : std::istream(0), mBinary(false), mReader(0) {}

av::InputStream::InputStream(std::istream& is) : std::istream(is.rdbuf()), mBinary(false), mReader(0) {}

av::InputStream::InputStream(std::streambuf* sb) : std::istream(sb), mBinary(false), mReader(0) {}

av::InputStream& av::InputStream::operator=(std::istream& is)
{
    (void)rdbuf(is.rdbuf());
    return *this;
}

av::InputStream& av::InputStream::operator=(std::streambuf* sb)
{
    (void)rdbuf(sb);
    return *this;
}

void av::InputStream::enableBinary(bool on) { mBinary = on; }

bool av::InputStream::isBinaryEnabled() const { return mBinary; }

void av::InputStream::setReader(av::Reader* reader) { mReader = reader; }

av::Reader* av::InputStream::getReader() { return mReader; }
...
av::InputStream& av::operator>>(av::InputStream& is, std::string& value)
{
    char str[1024];
    if(is.isBinaryEnabled())
    {
