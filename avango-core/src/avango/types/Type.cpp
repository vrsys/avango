...
av::Type::Type() { mTypeSpec.mId = 0; }

av::Type::Type(const Type& rhs) : mTypeSpec(rhs.mTypeSpec) {}

av::Type::Type(uint32_t id) { mTypeSpec.mId = id; }

av::Type::~Type() {}

const av::Type& av::Type::operator=(const Type& rhs)
{
    mTypeSpec = rhs.mTypeSpec;
    return *this;
}

/* static */ av::Type av::Type::badType() { return Type(0); }

av::Type::operator uint32_t() const { return mTypeSpec.mId; }

uint32_t av::Type::getId() const { return mTypeSpec.mId; }

unsigned int av::Type::getIndex() const { return mTypeSpec.mStorage.mIndex; }

av::Type av::Type::getParent() const { return type_data_vec[getIndex()].mParent; }

bool av::Type::operator==(const Type rhs) const { return mTypeSpec.mId == rhs.mTypeSpec.mId; }

bool av::Type::operator!=(const Type rhs) const { return !(*this == rhs); }

bool av::Type::operator<(const Type rhs) const { return mTypeSpec.mId < rhs.mTypeSpec.mId; }

bool av::Type::isBad() const { return (*this == badType()); }

bool av::Type::isPublic() const { return mTypeSpec.mStorage.mIsPublic; }

bool av::Type::canCreate() const { return mTypeSpec.mStorage.mCanCreate; }

const std::string& av::Type::getName() const { return type_data_vec[getIndex()].mName; }

/* static */ void av::Type::init()
{
    initRegistry();
...
/* static */ void av::Type::initRegistry()
{
...
/* static */ av::Type av::Type::getByName(const std::string& typeName)
{
...
/* static */ bool av::Type::isTypeRegistered(const std::string& typeName) { return getByName(typeName) != badType(); }

/* static */ av::Type av::Type::allocateNewType(bool isPublic, bool canCreate, bool isAdaptor)
{
    Type new_type;
...
/* static */ void av::Type::registerNewType(const std::string& typeName, Type parentType, Type newType, Create* creator)
{
    TypeData type_data;
...
av::Type av::Type::createAbstractType(Type parentType, const std::string& typeName, bool isPublic)
{
    ...
    if(parentType != badType() && parentType.getIndex() >= type_data_vec.size())
    {
        AVANGO_LOG(logger,
                   logging::WARN,
                   boost::str(boost::format("createAbstractType(): parent '%1%' of '%2%' doesn't exist yet (parent idx: %3%, max idx: %4%), returning 'badType'") % parentType.getName() % typeName %
    ...
    AVANGO_LOG(
        logger,
        logging::DEBUG,
        boost::str(boost::format("createAbstractType(): created abstract type '%s' (id: % 09d, idx: % 09d, parent: '%s').") % typeName % new_type.getId() % new_type.getIndex() % parentType.getName()))

    return new_type;
}

av::Type av::Type::createType(Type parentType, const std::string& typeName, Create* create, bool isPublic)
{
    // check for duplicates
    if(isTypeRegistered(typeName))
    {
    ...
    if(parentType != badType() && parentType.getIndex() >= type_data_vec.size())
    {
        AVANGO_LOG(logger,
                   logging::WARN,
                   boost::str(boost::format("createType(): parent '%1%' of '%2%' doesn't exist yet (parent idx: %3%, max idx: %4%), returning 'badType'") % parentType.getName() % typeName %
                              parentType.getIndex() % type_data_vec.size()))
    ...
    AVANGO_LOG(logger,
               logging::DEBUG,
               boost::str(boost::format("createType(): created type '%s' (id: % 09d, idx: % 09d, parent: '%s')") % typeName % new_type.getId() % new_type.getIndex() % parentType.getName()))

    return new_type;
}

void av::Type::setDistributable(bool isDistributable) { type_data_vec[getIndex()].mDistributable = isDistributable; }

bool av::Type::isDistributable() const { return type_data_vec[getIndex()].mDistributable; }

bool av::Type::isDerivedFrom(Type typeToCheck) const
{
...
bool av::Type::isOfType(Type typeToCheck) const { return (*this == typeToCheck || isDerivedFrom(typeToCheck)); }

av::TypeList av::Type::getAllDerivedFrom() const
{
...
av::Typed* av::Type::createInstance()
{
    if(!canCreate())
    {
        return 0;
...
/* static */ av::Typed* av::Type::createInstanceOfType(const std::string& typeName, bool /*unloadable*/)
{
...
/* static */ void av::Type::dumpTypeMap()
{
    ...
    for(TypeMap::const_iterator current_type = type_map.begin(); current_type != type_map.end(); ++current_type)
    {
    ...
    for(TypeMap::const_iterator current_type = type_map.begin(); current_type != type_map.end(); ++current_type)
    {
        ...
        AVANGO_LOG(logger,
                   logging::INFO,
                   boost::str(boost::format("%1%'%2%' idx: %3% [derivation: %4%] [public: %5%] [distributable: %6%]") % buffer % name % index % (!derivation.empty() ? derivation : "none") %
                              (current_type_data.mCreate ? "yes" : " no") % (current_type_data.mDistributable ? "yes" : " no")))
