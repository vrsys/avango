...
static const unsigned short sHidMappingConsumer[][3] = {{0x030, EV_KEY, KEY_POWER},
...
static const unsigned short sHidMappingValue[][4] = {
    {0x01, 0x30, EV_ABS, ABS_X}, {0x01, 0x31, EV_ABS, ABS_Y}, {0x01, 0x32, EV_ABS, ABS_Z}, {0x01, 0x33, EV_ABS, ABS_RX}, {0x01, 0x34, EV_ABS, ABS_RY}, {0x01, 0x35, EV_ABS, ABS_RZ}};
...
av::daemon::WindowsEventMapping::WindowsEventMapping()
{
    ...
    for(std::size_t i = 0; i < sizeof(sHidMappingConsumer) / sizeof(*sHidMappingConsumer); ++i)
        mHidMappingConsumer.insert(std::make_pair(sHidMappingConsumer[i][0], std::make_pair(sHidMappingConsumer[i][1], sHidMappingConsumer[i][2])));

    for(std::size_t i = 0; i < sizeof(sHidMappingValue) / sizeof(*sHidMappingValue); ++i)
        mHidMappingValue.insert(std::make_pair(std::make_pair(sHidMappingValue[i][0], sHidMappingValue[i][1]), std::make_pair(sHidMappingValue[i][2], sHidMappingValue[i][3])));
}

unsigned long av::daemon::WindowsEventMapping::mapKeyboardEvent(USHORT VKey)
{
...
bool av::daemon::WindowsEventMapping::mapHidButtonEvent(unsigned short usagePage, unsigned short usageId, unsigned short appUsagePage, unsigned short appUsageId, EventPair& ev)
{
    bool ret = false;
    switch(usagePage)
    {
...
bool av::daemon::WindowsEventMapping::mapHidValueEvent(unsigned short usagePage, unsigned short usageId, unsigned short appUsagePage, unsigned short appUsageId, EventPair& ev)
{
