...
bool
av::daemon::HIDInput::readEvent(struct input_event* event)
{
  if (isOpen() && event)
  {
    fd_set rfds;
    ...
    {
      case 0:
        logger.trace() << "read: select(2) timed out after " << timeout.tv_sec
                       << " sec for " << mTty.c_str();
        break;

      case -1:
        logger.warn() << "read: select(2) failed for " << mTty.c_str()
                      << " with reason: " << ::strerror(errno) << " (" << errno << ")";
         close();
         break;
...
bool
av::daemon::HIDInput::writeEvent(const struct input_event& event)
{
  if (isOpen())
  {
    ...
    switch (retval)
    {
      case sizeof(input_event):
        ...
        break;
      default:
        logger.warn() << "writeEvent: write(2) failed for " << mTty.c_str()
                      << " with reason: " << ::strerror(errno) << " (" << errno << ")";
        close();
        break;
      }
...
/* virtual */ void
av::daemon::HIDInput::retrieve_abs_info()
{
  ...
  {
    logger.warn() << "retrieve_abs_info: ioctl(EVIOCGBIT(EV_ABS)) failed with reason: "
                  << ::strerror(errno);
    return;
  }
  ...
  for (int loop = 0; loop < ABS_MAX; loop++) {
    if (TEST_BIT(loop, bitmask)) {
      ...
      {
        logger.warn() << "retrieve_abs_info: ioctl(EVIOCGABS) failed with reason: "
                      << ::strerror(errno);
      }
      else
      {
...
void
av::daemon::HIDInput::printAbsInfo() const
{
  ...
  while (current != mAbsInfoMap.end())
  {
    ...
    logger.info() << eventPairToString(EV_ABS, (*current).first).c_str() << " : "
...
float
av::daemon::HIDInput::normalizeAbsValue(const input_event& event) const
{
...
/* virtual */ void
av::daemon::HIDInput::updateLED(unsigned short eventCode, bool on)
{
  struct input_event event;
...
/* virtual */ void
av::daemon::HIDInput::updateLEDs(bool force)
{
