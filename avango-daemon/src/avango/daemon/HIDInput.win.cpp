...
av::daemon::HIDInput::HIDInput() : mUsagePage(0), mUsage(0), mDeviceNumber(1), mDevOpened(false) {}
...
std::string av::daemon::HIDInput::GetLastErrorString()
{
...
bool av::daemon::HIDInput::open()
{
    ...
    if(pos1 != std::string::npos)
    {
        ...
        {
            std::stringstream(mTty.substr(pos1 + delimiter.length(), pos2 - pos1 - delimiter.length())) >> std::hex >> mUsage;
    ...
    for(it = HIDInput::mAllDevices.begin(); it != HIDInput::mAllDevices.end(); ++it)
    {
        if((*it)->mUsage == mUsage && (*it)->mUsagePage == mUsagePage)
        {
            ...
            logger.info() << "open: reuse existing device handler with class name " << mHandler->mClassName << " for " << mTty.c_str();
            break;
        }
    }
    ...
    if(!handlerExists)
    {
        ...
        if(!RegisterClass(&wc))
        {
            logger.error() << "open: RegisterClass() failed for " << mTty.c_str() << " with reason: " << GetLastErrorString();
            return false;
        }
        ...
        if(!mHandler->hWnd)
        {
            logger.error() << "open: CreateWindow() failed for " << mTty.c_str() << " with reason: " << GetLastErrorString();
        ...
        logger.info() << "open: initialize new device handler with window class name " << mHandler->mClassName << " for " << mTty.c_str();
    }
...
bool av::daemon::HIDInput::isOpen() const { return mDevOpened; }
...
bool av::daemon::HIDInput::close()
{
    if(isOpen())
    {
        ...
        if(mHandler)
        {
            std::vector<HIDInput*>::iterator inpIt = std::find(mHandler->mInputs.begin(), mHandler->mInputs.end(), this);
            if(inpIt != mHandler->mInputs.end())
                mHandler->mInputs.erase(inpIt);

            if(!mHandler->mInputs.size())
            {
                DeviceHandlerArray::iterator handlerIt = std::find(HIDInput::mAllDevices.begin(), HIDInput::mAllDevices.end(), mHandler);
                ...
                if(!RegisterRawInputDevices(&mHandler->mRid, 1, sizeof(mHandler->mRid)))
                    logger.warn() << "close: RegisterRawInputDevices() failed upon closing " << mTty.c_str() << " with reason: " << GetLastErrorString();
                ...
                if(!UnregisterClass(mHandler->mClassName.c_str(), GetModuleHandle(NULL)))
                    logger.warn() << "close: UnregisterClass() failed upon closing " << mTty.c_str() << " with reason: " << GetLastErrorString();
                delete mHandler;
            }
            else
                logger.info() << "close: skipping device handler destruction for " << mTty.c_str();
            mHandler = 0;
        }
...
LRESULT CALLBACK av::daemon::HIDInput::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
...
av::daemon::HIDInput::DeviceHandler::initRawInput(HWND target)
{
        ...
        case RIM_TYPEHID:
            if(i > 0)
                for(long j = i - 1; j >= 0; --j)
                    if(devInfo[j].first.hid.usUsagePage == devInfo[i].first.hid.usUsagePage && devInfo[j].first.hid.usUsage == devInfo[i].first.hid.usUsage)
                    {
    ...
    boost::call_once(printDevListOnceFlag, [&]() {
        ...
        for(UINT i = 0; i < nDevices; ++i)
        {
            ...
            {
            case RIM_TYPEHID:
                str << "HID   " << std::hex << std::setw(2) << std::setfill('0') << info.hid.usUsagePage << "::" << std::setw(2) << info.hid.usUsage << "::" << number
                    << " vendorId=" << info.hid.dwVendorId << " productId=" << info.hid.dwProductId << std::dec << std::endl;
                break;

            case RIM_TYPEKEYBOARD:
                str << "KBD   "
                    << "01::06::" << number << " keys=" << info.keyboard.dwNumberOfKeysTotal << " funcKeys=" << info.keyboard.dwNumberOfFunctionKeys
                    << " indicators=" << info.keyboard.dwNumberOfIndicators << std::endl;
                break;

            case RIM_TYPEMOUSE:
                str << "MOUSE "
                    << "01::02::" << number << " dwId=" << info.mouse.dwId << " buttons=" << info.mouse.dwNumberOfButtons << " sampleRate=" << info.mouse.dwSampleRate << std::endl;
                break;
            }
        }
...
void av::daemon::HIDInput::reportHidButtonState(const HidButtonList& list)
{
    ...
    for(HidButtonList::const_iterator i = list.begin(); i != list.end(); ++i)
    {
        ...
        if(prIt == mPressedButtonList.end())
        {
            ...
            for(std::vector<unsigned short>::const_iterator j = i->second.begin(); j != i->second.end(); ++j)
            {
                ...
                logger.trace() << "reportHidButtonState: HID DOWN event: UsagePage: 0x" << std::hex << i->first << " Usage: 0x" << *j << std::dec << " // " << eventPairToString(ev.first, ev.second);
            }
        ...
        else
        {
            ...
            for(std::vector<unsigned short>::const_iterator j = i->second.begin(); j != i->second.end(); ++j)
                if(std::find(prIt->second.begin(), prIt->second.end(), *j) == prIt->second.end())
                {
                    ...
                    logger.trace() << "reportHidButtonState: HID DOWN event: UsagePage: 0x" << std::hex << i->first << " Usage: 0x" << *j << std::dec << " // "
            ...
            for(std::vector<unsigned short>::const_iterator j = prIt->second.begin(); j != prIt->second.end(); ++j)
                if(std::find(i->second.begin(), i->second.end(), *j) == i->second.end())
                {
                    ...
                    logger.trace() << "reportHidButtonState: HID UP event: UsagePage: 0x" << std::hex << i->first << " Usage: 0x" << *j << std::dec << " // " << eventPairToString(ev.first, ev.second);
                }
...
void av::daemon::HIDInput::parseRawInputHid(PRAWINPUT raw)
{
    UINT bufferSize = 0;

    if(GetRawInputDeviceInfo(raw->header.hDevice, RIDI_PREPARSEDDATA, NULL, &bufferSize) == (UINT)-1)
    {
        logger.warn() << "parseRawInputHid: GetRawInputDeviceInfo failed to get buffer size of preparsed data. Device: " << mTty.c_str();
        return;
    }
    ...
    {
        logger.warn() << "parseRawInputHid: GetRawInputDeviceInfo failed to retrieve preparsed data. Device: " << mTty.c_str();
        return;
    }
        ...
        if(HidP_GetButtonCaps(HidP_Input, &buttonCaps[0], &buttonCapsLength, preparsedData) == HIDP_STATUS_SUCCESS)
        {
            ...
            for(std::vector<HIDP_BUTTON_CAPS>::const_iterator capsIt = buttonCaps.begin(); capsIt != buttonCaps.end(); ++capsIt)
            {
                ...
                if(usageLength)
                {
                    ...
                    if(HidP_GetUsages(HidP_Input, capsIt->UsagePage, 0, &usageList[0], &usageLength, preparsedData, (PCHAR)raw->data.hid.bRawData, raw->data.hid.dwSizeHid) == HIDP_STATUS_SUCCESS)
                    {
    ...
    if(valueCapsLength)
    {
        ...
        if(HidP_GetValueCaps(HidP_Input, &valueCaps[0], &valueCapsLength, preparsedData) == HIDP_STATUS_SUCCESS)
        {
            for(std::size_t i = 0; i < valueCapsLength; ++i)
                ...
                {
                    ULONG value;
                    if(HidP_GetUsageValue(HidP_Input, valueCaps[i].UsagePage, 0, valueCaps[i].Range.UsageMin, &value, preparsedData, (PCHAR)raw->data.hid.bRawData, raw->data.hid.dwSizeHid) ==
                       HIDP_STATUS_SUCCESS)
                    {
                        ...
                        else
                        {
                            // map event
                            WindowsEventMapping::EventPair ev;
                            bool mapResult = WindowsEventMapping::mapHidValueEvent(valueCaps[i].UsagePage, valueCaps[i].Range.UsageMin, mUsagePage, mUsage, ev);

                            if(mapResult)
                            {
                                float normValue = signedValue;
                                if(mNormAbs && ev.first == EV_ABS)
                                    normValue = 2.0 * (signedValue - valueCaps[i].LogicalMin) / (float)(valueCaps[i].LogicalMax - valueCaps[i].LogicalMin + 1) - 1.0;

                                events.push_back(ConstructEvent(ev.first, ev.second, signedValue, normValue));
                            }
                            else
                                logger.warn() << "parseRawInputHid: can't map event for UsagePage: 0x" << std::hex << valueCaps[i].UsagePage << ", UsageId: 0x" << valueCaps[i].Range.UsageMin
...
void av::daemon::HIDInput::parseRawInputMouse(PRAWINPUT raw)
{
...
void av::daemon::HIDInput::parseRawInputKeyboard(PRAWINPUT raw)
{
...
unsigned short av::daemon::HIDInput::getDeviceNumber(HANDLE h) const
{
...
void av::daemon::HIDInput::stopDevice() { logger.info() << "stopDevice: done."; }
