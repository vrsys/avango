...
av::daemon::TUIOInput::TUIOInput()
    : mTUIOClient(nullptr), mTUIOInputListener(std::make_shared<TUIOInputListener>()), mRequiredFeatures(), mStationToSessionID_cursors(), mStationToSessionID_fingers(), mStationToSessionID_hands()
{
    mRequiredFeatures.push_back("port");
}

void av::daemon::TUIOInput::initClass()
{
...
/* virtual */ void av::daemon::TUIOInput::startDevice()
{
    if(!parseFeatures())
        return;
...
int av::daemon::TUIOInput::getSessionIDForStation_cursors(std::pair<int, av::daemon::Station*> const& station)
{
    ...
    if(left_it != mStationToSessionID_cursors.left.end() && cursors.find(left_it->second) == cursors.end())
    {
    ...
    if(left_it == mStationToSessionID_cursors.left.end())
    {
        ...
        {
            auto right_it(mStationToSessionID_cursors.right.find(cursor.second.session_id));
            if(right_it == mStationToSessionID_cursors.right.end())
            {
                mStationToSessionID_cursors.insert(::boost::bimap<int, int>::value_type(station.first, cursor.second.session_id));
...
int av::daemon::TUIOInput::getSessionIDForStation_fingers(std::pair<int, av::daemon::Station*> const& station)
{
    auto fingers = mTUIOInputListener->fingers();
    auto left_it(mStationToSessionID_fingers.left.find(station.first));

    // remove mapping if session ID has expired
    if(left_it != mStationToSessionID_fingers.left.end() && fingers.find(left_it->second) == fingers.end())
    {
    ...
    if(left_it == mStationToSessionID_fingers.left.end())
    {
        ...
        {
            auto right_it(mStationToSessionID_fingers.right.find(finger.second.session_id));
            if(right_it == mStationToSessionID_fingers.right.end())
            {
                mStationToSessionID_fingers.insert(::boost::bimap<int, int>::value_type(station.first, finger.second.session_id));
...
int av::daemon::TUIOInput::getSessionIDForStation_hands(std::pair<int, av::daemon::Station*> const& station)
{
    auto hands = mTUIOInputListener->hands();
    auto left_it(mStationToSessionID_hands.left.find(station.first));

    // remove mapping if session ID has expired
    if(left_it != mStationToSessionID_hands.left.end() && hands.find(left_it->second) == hands.end())
    {
    ...
    if(left_it == mStationToSessionID_hands.left.end())
    {
        ...
        {
            auto right_it(mStationToSessionID_hands.right.find(hand.second.session_id));
            if(right_it == mStationToSessionID_hands.right.end())
            {
                mStationToSessionID_hands.insert(::boost::bimap<int, int>::value_type(station.first, hand.second.session_id));
...
/* virtual */ void av::daemon::TUIOInput::readLoop()
{
#ifdef AVANGO_DAEMON_DEBUG
...
/* virtual */ void av::daemon::TUIOInput::stopDevice()
{
    mTUIOClient->disconnect();
    logger.info() << "stopDevice: done.";
}

const std::vector<std::string>& av::daemon::TUIOInput::queryFeatures() { return mRequiredFeatures; }

bool av::daemon::TUIOInput::parseFeatures()
{
