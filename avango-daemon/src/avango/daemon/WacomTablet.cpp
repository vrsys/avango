...
void av::daemon::WacomTablet::initClass()
{
...
void av::daemon::WacomTablet::startDevice()
{
    // auto-configure axes
...
void av::daemon::WacomTablet::retrieveAspectRatio()
{
...
/* virtual */ void av::daemon::WacomTablet::readLoop()
{
    float x, y;
...
float av::daemon::WacomTablet::normalizeAbsValue(const input_event& event) const
{
    ...
    case ABS_RY:
        if(event.value > 0.0)
            normalized_value = log(event.value) / log(2) + 1;
        else
...
/*virtual*/ int av::daemon::WacomTablet::parse_features()
{
