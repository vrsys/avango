...
class TuioPoint
{
  ...
  public:
     ...
     */
    float getDistance(TuioPoint* tpoint) { return getDistance(tpoint->getX(), tpoint->getY()); }
     ...
     */
    float getAngle(TuioPoint* tpoint) { return getAngle(tpoint->getX(), tpoint->getY()); }
     ...
     */
    float getAngleDegrees(float xp, float yp) { return ((getAngle(xp, yp) / (float)M_PI) * 180.0f); }
     ...
     * @return	the angle in degrees to the provided TuioPoint
     */
    float getAngleDegrees(TuioPoint* tpoint) { return ((getAngle(tpoint) / (float)M_PI) * 180.0f); }
     ...
     */
    int getScreenX(int width) { return (int)floor(xpos * width + 0.5f); };
     ...
     */
    int getScreenY(int height) { return (int)floor(ypos * height + 0.5f); };
     ...
     */
    TuioTime getTuioTime() { return currentTime; };
     ...
     */
    TuioTime getStartTime() { return startTime; };
};
}; // namespace TUIO
#endif
