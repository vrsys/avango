...
TuioServer::TuioServer() { initialize("127.0.0.1", 3333, MAX_UDP_SIZE); }

TuioServer::TuioServer(const char* host, int port) { initialize(host, port, IP_MTU_SIZE); }

TuioServer::TuioServer(const char* host, int port, int size) { initialize(host, port, size); }
...
void TuioServer::updateTuioObject(TuioObject* tobj, float x, float y, float a)
{
    ...
    if(verbose && tobj->isMoving())
        std::cout << "set obj " << tobj->getSymbolID() << " (" << tobj->getSessionID() << ") " << tobj->getX() << " " << tobj->getY() << " " << tobj->getAngle() << " " << tobj->getXSpeed() << " "
                  << tobj->getYSpeed() << " " << tobj->getRotationSpeed() << " " << tobj->getMotionAccel() << " " << tobj->getRotationAccel() << std::endl;
}
...
void TuioServer::updateTuioCursor(TuioCursor* tcur, float x, float y)
{
    ...
    if(verbose && tcur->isMoving())
        std::cout << "set cur " << tcur->getCursorID() << " (" << tcur->getSessionID() << ") " << tcur->getX() << " " << tcur->getY() << " " << tcur->getXSpeed() << " " << tcur->getYSpeed() << " "
                  << tcur->getMotionAccel() << " " << std::endl;
}
...
long TuioServer::getFrameID() { return currentFrame; }

TuioTime TuioServer::getFrameTime() { return currentFrameTime; }
...
std::list<TuioObject*> TuioServer::getTuioObjects() { return objectList; }

std::list<TuioCursor*> TuioServer::getTuioCursors() { return cursorList; }
