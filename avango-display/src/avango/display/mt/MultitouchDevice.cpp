...
void
av::display::mt::MultitouchDevice::processMessages(std::queue< std::pair< FingerClient::MyMessage, int > > &messages)
{
...
void
av::display::mt::MultitouchDevice::handleFingers()
{
...
void
av::display::mt::MultitouchDevice::handleUsers()
{
...
void
av::display::mt::MultitouchDevice::userAdded(UserInfo *pUserInfo)
{
  boost::mutex::scoped_lock mLock(mMutex);
  // FIXME: Between two frames (evaluate() calls), it can happen, that a finger id
  // is added again, before the former one was removed completely.
...
void
av::display::mt::MultitouchDevice::userMoved(UserInfo *pUserInfo)
{
  boost::mutex::scoped_lock mLock(mMutex);
  const int user_id = pUserInfo->getUserId();
  UserMap::iterator iter = mUserMap.find(user_id);
...
void
av::display::mt::MultitouchDevice::userRemoved(UserInfo *pUserInfo)
{
  boost::mutex::scoped_lock mLock(mMutex);
  const int user_id = pUserInfo->getUserId();
  UserMap::iterator iter = mUserMap.find(user_id);
