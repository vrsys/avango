...
class AV_GUA_DLL LineStripLoader : public av::FieldContainer
{
  ...
  public:
     ...
     *
     */
    av::Link<av::gua::Node> createEmptyGeometry(std::string const& nodeName, std::string const& emptyName, av::gua::Material const& fallbackMaterial, unsigned flags = DEFAULTS) const;

    /**
     *
     */
    av::Link<av::gua::Node> createEmptyGeometry(std::string const& nodeName, std::string const& emptyName, unsigned flags = DEFAULTS) const;
