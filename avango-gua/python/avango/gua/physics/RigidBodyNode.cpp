...
void applyForce(av::gua::RigidBodyNode const& node, ::gua::math::vec3 const& force, ::gua::math::vec3 const& relative_position) { node.getGuaNode()->apply_force(force, relative_position); }

void applyCentralForce(av::gua::RigidBodyNode const& node, ::gua::math::vec3 const& force) { node.getGuaNode()->apply_central_force(force); }

void applyTorque(av::gua::RigidBodyNode const& node, ::gua::math::vec3 const& torque) { node.getGuaNode()->apply_torque(torque); }

void applyTorqueImpulse(av::gua::RigidBodyNode const& node, ::gua::math::vec3 const& torque) { node.getGuaNode()->apply_torque_impulse(torque); }

void applyImpulse(av::gua::RigidBodyNode const& node, ::gua::math::vec3 const& impulse, ::gua::math::vec3 const& relative_position) { node.getGuaNode()->apply_impulse(impulse, relative_position); }

void applyCentralImpulse(av::gua::RigidBodyNode const& node, ::gua::math::vec3 const& impulse) { node.getGuaNode()->apply_central_impulse(impulse); }

void clearForces(av::gua::RigidBodyNode const& node) { node.getGuaNode()->clear_forces(); }
...
void init_RigidBodyNode()
{
    ...
    class_<av::gua::RigidBodyNode, av::Link<av::gua::RigidBodyNode>, bases<av::gua::TransformNode>, boost::noncopyable>("RigidBodyNode", "docstring", no_init)
