...
void addMemorySegment(av::gua::NamedSharedMemoryController& memory_controller, std::string const& segment_name, int memory_chunk_size_in_bytes)
{
    memory_controller.add_memory_segment(segment_name, memory_chunk_size_in_bytes);
}

void addReadOnlyMemorySegment(av::gua::NamedSharedMemoryController& memory_controller, std::string const& segment_name) { memory_controller.add_read_only_memory_segment(segment_name); }

void constructNamedAtomicIntOnSegment(av::gua::NamedSharedMemoryController& memory_controller, std::string const& segment_name, std::string const& object_name)
{
    memory_controller.construct_named_object_on_segment<std::atomic_int>(segment_name, object_name);
}

void constructNamed2KBCharBufferOnSegment(av::gua::NamedSharedMemoryController& memory_controller, std::string const& segment_name, std::string const& object_name)
{
    memory_controller.construct_named_object_on_segment<std::array<char, ::gua::MemAllocSizes::KB2>>(segment_name, object_name);
}

void constructNamed64KBCharBufferOnSegment(av::gua::NamedSharedMemoryController& memory_controller, std::string const& segment_name, std::string const& object_name)
{
    memory_controller.construct_named_object_on_segment<std::array<char, ::gua::MemAllocSizes::KB64>>(segment_name, object_name);
}

void constructNamed1MBCharBufferOnSegment(av::gua::NamedSharedMemoryController& memory_controller, std::string const& segment_name, std::string const& object_name)
{
    memory_controller.construct_named_object_on_segment<std::array<char, ::gua::MemAllocSizes::MB1>>(segment_name, object_name);
}

void constructNamed16MBCharBufferOnSegment(av::gua::NamedSharedMemoryController& memory_controller, std::string const& segment_name, std::string const& object_name)
{
    memory_controller.construct_named_object_on_segment<std::array<char, ::gua::MemAllocSizes::MB16>>(segment_name, object_name);
}

void setValueForNamedAtomicIntOnSegment(av::gua::NamedSharedMemoryController& memory_controller, std::string const& object_name, int value)
{
...
int getValueFromNamedAtomicIntOnSegment(av::gua::NamedSharedMemoryController& memory_controller, std::string const& object_name)
{
    // first template parameter: internal_type, second template parameter: external_type
    return memory_controller.get_value_from_named_object<std::atomic_int, int>(object_name);
}

void registerRemotelyConstructedObjectOnSegment(av::gua::NamedSharedMemoryController& memory_controller, std::string const& segment_name, std::string const& object_name)
{
...
void init_NamedSharedMemoryController()
{
    ...
    class_<av::gua::NamedSharedMemoryController, av::Link<av::gua::NamedSharedMemoryController>, bases<av::FieldContainer>, boost::noncopyable>("NamedSharedMemoryController", "docstring", no_init);
