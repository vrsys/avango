...
std::shared_ptr<::gua::NamedSharedMemoryController> av::gua::NamedSharedMemoryController::m_guaNamedSharedMemoryController = ::gua::NamedSharedMemoryController::instance_shared_ptr();
...
void av::gua::NamedSharedMemoryController::initClass()
{
...
std::shared_ptr<::gua::NamedSharedMemoryController> av::gua::NamedSharedMemoryController::getGuaNamedSharedMemoryController() const { return m_guaNamedSharedMemoryController; }

#if not defined(__WIN32__) && not defined(_WIN32) && not defined(_WIN64)
void av::gua::NamedSharedMemoryController::add_memory_segment(std::string const& segment_name, uint64_t size_in_byte)
{
    m_guaNamedSharedMemoryController->add_memory_segment(segment_name, size_in_byte);
}

void av::gua::NamedSharedMemoryController::add_read_only_memory_segment(std::string const& segment_name) { m_guaNamedSharedMemoryController->add_read_only_memory_segment(segment_name); }

void av::gua::NamedSharedMemoryController::register_remotely_constructed_object_on_segment(std::string const& segment_name, std::string const& object_name)
{
