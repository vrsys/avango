...
av::gua::BlenderViewer::Image screenshot(scm::gl::render_context_ptr const& ctx, scm::gl::texture_2d_ptr const& tex)
{
...
void draw_image(av::gua::BlenderViewer::Image const& im, GLuint texid, bool resize)
{
    ...
    if(resize)
    {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, im.width, im.height, 0, im.gl_base_format, im.gl_type, im.data.data());
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    }
    else
    {
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, im.width, im.height, im.gl_base_format, im.gl_type, im.data.data());
    }
...
av::gua::BlenderViewer::BlenderViewer()
    : m_mutex(), m_condition(), m_gua_graphs(), m_current_engine(""), m_camera_name(""), m_image(), m_ready(false), m_processed(false), m_done(false),
      m_worker(std::bind(&av::gua::BlenderViewer::render_thread, this)), m_engines(), m_unregister_queue()
{
    ...
    while(keep_running)
    {
        ...
        if(window && window->get_is_open())
        {
            if(!m_gua_graphs.empty())
            {
                ...
                auto it = std::find_if(
                    cams.begin(), cams.end(), [this](::gua::node::CameraNode* const& c) -> bool { return "blender_window" == c->config.get_output_window_name() && m_camera_name == c->get_name(); });
                if(it != cams.end())
                {
                    auto& cam = *it;
                    ...
                    {
                        pipe = std::make_shared<::gua::Pipeline>(*window->get_context(), serialized_cam.config.get_resolution());
...
av::gua::BlenderViewer::Image av::gua::BlenderViewer::screenshot(::gua::Pipeline& pipe)
{
    ...
    if(!tmp.rgba8_texture || tmp.rgba8_texture->descriptor()._size != tex->descriptor()._size)
    {
        ...
        tmp.rgba8_texture = ctx.render_device->create_texture_2d(tex->descriptor()._size, scm::gl::FORMAT_RGBA_8);
