...
/* virtual */ void
av::osg::LineSegmentIntersector::evaluate()
{
...
/* virtual */ void
av::osg::LineSegmentIntersector::fieldHasChanged(const av::Field& field)
{
...
std::vector<av::Link<av::osg::Node> >
av::osg::LineSegmentIntersector::getAvangoNodePath(::osgUtil::LineSegmentIntersector::Intersections i)
{
  // ToDo: Enhance method to return node path of all intersected nodes
  std::vector<av::Link<av::osg::Node> > nodepath;

  for(osgUtil::LineSegmentIntersector::Intersections::const_iterator iter = i.begin();
      iter != i.end();
      ++iter)
  {
    const ::osg::NodePath np = iter->nodePath;

    for(::osg::NodePath::const_iterator p_iter = np.begin();
        p_iter != np.end();
        ++p_iter)
    {
