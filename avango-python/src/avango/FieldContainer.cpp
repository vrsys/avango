...
std::string get_type_name(av::FieldContainer* self) { return self->getTypeId().getName(); }

bool is_derived_from(av::FieldContainer* self, std::string name) { return self->getTypeId().isDerivedFrom(av::Type::getByName(name)); }
...
av::Field* make_field_by_name(std::string name) { return dynamic_cast<av::Field*>(av::Type::createInstanceOfType(name)); }

av::Link<av::FieldContainer> ContainerPoll_get_instance_by_name(std::string name) { return av::Link<av::FieldContainer>(av::ContainerPool::getInstanceByName(name)); }

unsigned int getNumberOfContainersToEvaluate() { return av::FieldContainer::getNumberOfContainersToEvaluate(); }

bool Base_equal(const av::Base* a, const av::Base* b) { return a == b; }

bool Base_unequal(const av::Base* a, const av::Base* b) { return a != b; }

void FieldContainer_touch(av::FieldContainer* self) { self->touch(); }
...
{
  public:
    FieldContainerLink(av::Link<av::FieldContainer> const& container) : mLink(container) {}

    av::FieldContainer* getPtr(void) const { return mLink.getPtr(); }

  private:
    av::Link<av::FieldContainer> mLink;
};

av::FieldContainer* get_pointer(const FieldContainerLink& link) { return link.getPtr(); }
...
{
template <>
struct pointee<FieldContainerLink>
{
...
void init_FieldContainer(void)
{
    ...
    class_<av::Base, av::Link<av::Base>, boost::noncopyable>("Base", "docstring", no_init).def("__eq__", Base_equal).def("__ne__", Base_unequal);
