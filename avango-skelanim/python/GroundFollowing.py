...
class GroundFollowing(avango.script.Script):
    ...
    def my_constructor(self, gravity=-9.81,
                       init_matrix_out=avango.gua.make_identity_mat()):
    ...
    def evaluate(self):
        ...
        self.__ray.Transform.value = \
            avango.gua.make_trans_mat(in_translation) *\
        ...
        if len(results.value) > 0:
            ...
            new_pos = avango.gua.make_trans_mat(hit_world_trans)
            # * avango.gua.make_trans_mat(0.0,self.OffsetToGround.value, 0.0)

            delta_trans = (new_pos.get_translate() -
                           self.InTransform.value.get_translate())

            self.DistanceToGround.value = delta_trans.y
        else:
            self.DistanceToGround.value = self.MaxDistanceToGround.value + 1

        delta_norm = avango.gua.Vec3(
            delta_trans.x,
            delta_trans.y,
            delta_trans.z)
        ...
        if length > math.fabs(self._velocity_y):
            ...
            self.OutTransform.value = \
                self.OutTransform.value * \
                avango.gua.make_trans_mat(delta_norm*self._velocity_y)

        else:

            self.OutTransform.value = \
                self.OutTransform.value * \
                avango.gua.make_trans_mat(delta_trans)
