...
av::sound::SoundSource::SoundSource() : mPollPlaying(false)
{
    AV_FC_ADD_FIELD(URL, "");
...
/*static*/ void av::sound::SoundSource::initClass()
{
...
void av::sound::SoundSource::for_each_local_source(MemFun f)
{
    for(LocalSources::iterator local_source = mLocalSources.begin(); local_source != mLocalSources.end(); ++local_source)
    {
...
void av::sound::SoundSource::for_each_local_source(MemFun f, Arg arg)
{
    for(LocalSources::iterator local_source = mLocalSources.begin(); local_source != mLocalSources.end(); ++local_source)
    {
        LocalSource& obj = **local_source;
...
/* virtual */ void av::sound::SoundSource::fieldHasChangedLocalSideEffect(const Field& field)
{
    if(&field == &URL)
    {
...
void av::sound::SoundSource::initializeLocalSource(boost::shared_ptr<SoundSource::LocalSource> localSource)
{
...
void av::sound::SoundSource::evaluateLocalSideEffect()
{
    if(!mPollPlaying)
    {
        return;
...
void av::sound::SoundSource::addLocalSource(boost::shared_ptr<SoundSource::LocalSource> localSource)
{
    initializeLocalSource(localSource);
    mLocalSources.push_back(localSource);
}

void av::sound::SoundSource::removeLocalSource(boost::shared_ptr<SoundSource::LocalSource> localSource)
{
    mLocalSources.erase(std::remove(mLocalSources.begin(), mLocalSources.end(), localSource), mLocalSources.end());
}

unsigned int av::sound::SoundSource::numLocalSources() const { return mLocalSources.size(); }

boost::shared_ptr<av::sound::SoundSource::LocalSource> av::sound::SoundSource::getLocalSource(unsigned int index) { return mLocalSources.at(index); }

av::sound::SoundSource::LocalSource::LocalSource(SoundRenderer* renderer) : mSoundRenderer(renderer) {}

/* virtual */
av::sound::SoundSource::LocalSource::~LocalSource() {}

const av::sound::SoundRenderer* av::sound::SoundSource::LocalSource::getRenderer() { return mSoundRenderer; }
