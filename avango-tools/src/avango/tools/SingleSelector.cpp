...
void av::tools::SingleSelector::initClass()
{
...
/* virtual */ void av::tools::SingleSelector::evaluate()
{
    av::tools::Selector::evaluate();

    if(!Targets.isEmpty())
    {
        SelectedTargets.setValue(MFTargetHolder::ContainerType(1, LastTarget.getValue() ? Targets.getValue().back() : Targets.getValue().front()));
    }
