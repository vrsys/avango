...
class AV_GUA_DLL TV_3Loader : public av::FieldContainer
{
  ...
  public:
    ...
    av::Link<av::gua::Node> load(std::string const& nodeName, std::string const& fileName, Flags flags = DEFAULTS, int const cpu_budget = 1024, int const gpu_budget = 1024) const;
    av::Link<av::gua::Node>
    load(std::string const& nodeName, std::string const& fileName, av::gua::Material const& fallbackMaterial, Flags flags = DEFAULTS, int const cpu_budget = 1024, int const gpu_budget = 1024) const;
