...
av::Link<av::gua::Node> load(av::gua::VolumeLoader const& loader, std::string const& nodename, std::string const& fileName, int flags)
{
    return loader.load(nodename, fileName, static_cast<av::gua::VolumeLoader::Flags>(flags));
}

void init_VolumeLoader()
{
    class_<av::gua::VolumeLoader, av::Link<av::gua::VolumeLoader>, bases<av::FieldContainer>, boost::noncopyable>("VolumeLoader", "docstring", no_init)
        .def("load", &load)
