...
def add_lights(graph, count):
    ...
    for x in range(0, count):
        ...
        graph.Root.value.Children.value.append(avango.gua.nodes.TransformNode(
            Name="light" + str(x),
            Transform=avango.gua.make_trans_mat(
                randdir[0], randdir[1], randdir[2]
                ),
...
def setup_scene(graph, root_monkey, depth_count):
    ...
    if (depth_count > 0):
        ...
        for direction in directions:
            ...
            monkey_geometry.Transform.value = avango.gua.make_trans_mat(
