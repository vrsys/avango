...
# schism
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/opt/schism/current/lib/linux_x86:/opt/Awesomium/lib:/opt/lamure/install/lib
...
then
    python3 $LOCAL_AVANGO/examples/daemon/hmd-zmq-daemon.py > /dev/null &
else
    python3 $AVANGO/examples/examples_common/daemon.py > /dev/null &
fi

# run program
cd "$DIR" && python3 ./hmd-zmq-app.py
