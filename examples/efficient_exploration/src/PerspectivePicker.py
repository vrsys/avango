  ...
  def find_closest_perspective(self):
    ...
    for tup in angle_list:
      ...
      if tup[0].frustum.contains(lense_pos):
        ...
        # print('pnpp_dot',pnpp_dot)
        # pnpp_dot = 1 if pnpp_dot >= 1 else pnpp_dot
        # pnpp_dot = -1 if pnpp_dot <= -1 else pnpp_dot

        pnlp_dot = plane_n.x*line_p.x + plane_n.y*line_p.y + plane_n.z*line_p.z
        # print('pnlp_dot',pnlp_dot)
        # pnlp_dot = 1 if pnlp_dot >= 1 else pnlp_dot
        # pnlp_dot = -1 if pnlp_dot <= -1 else pnlp_dot

        pnld_dot = plane_n.x*line_d.x + plane_n.y*line_d.y + plane_n.z*line_d.z
        # print('pnld_dot',pnld_dot)
        # pnld_dot = 1 if pnld_dot >= 1 else pnld_dot
        # pnld_dot = -1 if pnld_dot <= -1 else pnld_dot
        ...
        x_values = [plane_p.x - _img.img_w_half, plane_p.x + _img.img_w_half]
        y_values = [plane_p.y - _img.img_h_half, plane_p.y + _img.img_h_half]
        ...
        if u_coord > _img.min_uv.x and u_coord < _img.max_uv.x:
          print('yes')
        zoom_factor = 1.5
        max_uv = avango.gua.Vec2(u_coord - (_img.tile_w / 2 / zoom_factor) , v_coord - (_img.tile_w / 2 / zoom_factor))
        # min_uv = avango.gua.Vec2(u_coord - (_img.tile_w / 2 / zoom_factor) , v_coord - (_img.tile_h / 2 / zoom_factor))
        min_uv = avango.gua.Vec2(u_coord + (_img.tile_w / 2 / zoom_factor) , v_coord + (_img.tile_w / 2 / zoom_factor))
