...
class TimedFPSPrinter(avango.script.Script):
  ...
  def update(self):
    ...
    if self.WindowLeft != 0:
      print("RenderingFPS L: " + str(self.WindowLeft.RenderingFPS.value) )

    if self.WindowCenter != 0:
      if len(self.LoggingIndicatorNode.Tags.value) > 0:
        print("RenderingFPS C: " + str(self.WindowCenter.RenderingFPS.value) )

    if (self.AvatarNode != 0) and (self.WindowCenter != 0):
      if len(self.LoggingIndicatorNode.Tags.value) > 0:
        ...
        if not current_stream_timestamp in self.entry_count_dict:
          self.draw_time_dict[current_stream_timestamp] = 0.0
        ...
        self.draw_time_dict[current_stream_timestamp] += current_draw_time_center
        ...
        print("#Tris " + str(self.AvatarNode.get_stats_num_triangles()) )
        print("RC Timestamp s:" + str(self.AvatarNode.get_stats_timestamp()) )
        print("RC Recon Time ms: " + str(self.AvatarNode.get_stats_reconstruction_time()) )
        print("Current draw time center: " + str(current_draw_time_center) )
        ...
        if self.num_entries > 0:
          ...
          log_draw_times = open("log_draw_times_mode_" + str(self.num_files_logged) , 'w')
          for key, value in sorted(self.entry_count_dict.items()):
            log_draw_times.write(str(key) + " " + str(self.draw_time_dict[key] / value) + "\n" )
          log_draw_times.close()

          self.draw_time_dict.clear()
...
class Initializer(avango.script.Script):
  def __init__(self):
    ...
    self.window_left = avango.gua.nodes.Window(Size=size,
                                      Display = ":0.3",  # ":0.1",
    ...
    self.window_center = avango.gua.nodes.Window(Size=size,
                                 Display = ":0.1",  # ":0.1",
                                 LeftPosition = left_pos,
                                 RightPosition = right_pos,
                                 LeftResolution=rendering_res,
    ...
    self.window_right = avango.gua.nodes.Window(Size=size,
                                      Display = ":0.0",  # ":0.1",
                                      LeftPosition = left_pos,
                                      RightPosition = right_pos,
                                      LeftResolution=rendering_res,
