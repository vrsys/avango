...
scene_root = avango.osg.nodes.Group(Name="SceneRoot", Children=[catch_all_sphere])
object_root = avango.osg.nodes.Group(Name="ObjectRoot")
scene_root.Children.value.append(object_root)


#create an inspector
inspector = avango.inspector.nodes.Inspector(Children=[object_root])

# Setup viewer
window = avango.osg.viewer.nodes.GraphicsWindow(NumStencilBits=1, Title = "AVANGO Inspector example")
camera = avango.osg.viewer.nodes.Camera(Window=window)
camera.ViewerTransform.value = avango.osg.make_trans_mat( avango.osg.Vec3(0,0,30) )
...
pick_selector.FirstHitOnly.value = False

name_selector = avango.tools.nodes.NameSelector(SelectableNames=['dragObject'])
name_selector.EqualNameOnly.value = False
name_selector.SearchOSGNodePaths.value = True

type_selector = avango.tools.nodes.TypeSelector()
type_selector.SelectableTypes.value.append(avango.osg.nodes.MatrixTransform())
...
class CatchAllSphereContextBehavior(avango.script.Script):
    ...
    def show_context_menu(self, target_holder=None):
        ...
        # add buttons to menu
        context_menu.add_widget(self.create_cow_button)
    ...
    def create_cow_changed(self):
        ...
        if self.old_cow_state and not self.CreateCow.value:
            panel_group.hide_panels()

        self.old_cow_state = self.CreateCow.value
    ...
    def create_panel_changed(self):
        if self.CreatePanel.value:
            new_panel = avango.osg.nodes.Panel(BorderWidth=0.001,
                                               ...
                                               EdgeRadius=0.002)
            new_geode = avango.osg.nodes.Geode(Drawables=[new_panel], Name="PanelGeode"+str(self.actual_object_num))
            new_geode.add_and_init_field(avango.SFBool(), "PickIgnore", True)
            new_transform = avango.osg.nodes.MatrixTransform(Children=[new_geode], Name="dragObject"+str(self.actual_object_num))
...
class ObjectBehavior(avango.script.Script):
    ...
    def __init__(self):
        self.radius_slider = avango.menu.widget.Slider(Min=0.5, Max=5, Title="Radius")
    ...
    def delete_object_changed(self):
        print "A Delete object"
        # react on butten release: close panel and delete object
        if self.delete_old_state and not self.DeleteObject.value:
            scene_graph_parent = self.hit_object.Parents.value[0].Parents.value[0]
            print "hitObjectName: " + str(scene_graph_parent.Name.value)
