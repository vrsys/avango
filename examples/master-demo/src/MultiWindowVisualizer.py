...
class MultiWindowVisualizer(avango.script.Script):
    ...
    def my_constructor(self, parent_node, atlas_path, images, width, height, rows=4, columns=4):
        ...
        for idx, img in enumerate(self.selected_images):
            ...
            # rot = avango.gua.make_rot_mat(180.0,0.0,0.0,1.0)
            ...
            pos = transform * avango.gua.Vec3( w_half, h_half, 0.0) * rot
            uv  = avango.gua.Vec2(x_min, y_min)
            t2_v1 = ImageVertex(self.dynamic_triangle_node, idx * 6 + 3, pos, uv)
            
            pos = transform * avango.gua.Vec3(-w_half, -h_half, 0.0) * rot
            uv  = avango.gua.Vec2(x_max, y_max)
            t2_v2 = ImageVertex(self.dynamic_triangle_node, idx * 6 + 4, pos, uv)
            
            pos = transform * avango.gua.Vec3(w_half, -h_half, 0.0) * rot
            uv  = avango.gua.Vec2(x_min, y_max)
            t2_v3 = ImageVertex(self.dynamic_triangle_node, idx * 6 + 5, pos, uv)

            self.min_uv = avango.gua.Vec2(x_min, y_min)
            self.max_uv = avango.gua.Vec2(x_max, y_max)
    ...
    def update_images(self, image_id_list, tex_coords):
        ...
        for idx, img_id in enumerate(image_id_list):
            ...
            uv2  = avango.gua.Vec2(tex_coords[idx][1].x, tex_coords[idx][0].y)
            ...
            uv5  = avango.gua.Vec2(tex_coords[idx][1].x, tex_coords[idx][1].y)
