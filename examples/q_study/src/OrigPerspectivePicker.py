  ...
  def find_closest_perspective(self):
    ...
    for tup in angle_list:
      ...
      if tup[0].frustum.contains(lense_pos):
        ...
        x_values = [plane_p.x - _img.tile_width, plane_p.x + _img.tile_width]
        y_values = [plane_p.y - _img.tile_height, plane_p.y + _img.tile_height]

        # print(_img.img_h_half, y_values[1] - y_values[0])
        # ratio = 4.07 / 2.30
        # print('uv',_img.min_uv.x, _img.max_uv.x)

        test_u_coord = map_from_to(intersection_pos.x, x_values[0], x_values[1], 1.0, 0.0)
        # print('map x', intersection_pos.x,'to', x_values[0], x_values[1],test_u_coord)

        test_v_coord = map_from_to(intersection_pos.y, y_values[0], y_values[1], 1.0, 0.0)
        # print('map y', intersection_pos.y,'to', y_values[0], y_values[1],test_v_coord)

        # u_coord = map_from_to(intersection_pos.x, x_values[0], x_values[1], _img.min_uv.x, _img.max_uv.x)
        u_coord = map_from_to(test_u_coord, 0.0,1.0, _img.min_uv.x, _img.max_uv.x)
        v_coord = map_from_to(test_v_coord, 0.0,1.0, _img.min_uv.y, _img.max_uv.y)

        # u_coord = map_from_to(intersection_pos.x, x_values[0], x_values[1], _img.min_uv.x, _img.max_uv.x)
        # v_coord = map_from_to(intersection_pos.y, y_values[0], y_values[1], _img.min_uv.y, _img.max_uv.y)

        # print(_img.id, 'ORG uv coords min max', _img.min_uv.x, _img.max_uv)
