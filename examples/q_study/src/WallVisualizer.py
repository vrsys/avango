...
class WallVisualizer(avango.script.Script):

    def __init__(self):
        self.super(WallVisualizer).__init__()
    ...
    def my_constructor(self, parent_node, atlas_path, images, width, height, rows=3, columns=4):
        ...
        for idx, img in enumerate(self.selected_images):
            ...
            transform = avango.gua.make_trans_mat(x, y, 0.0) * avango.gua.make_rot_mat(90.0,0.0,0.0,1.0)
            # rot = avango.gua.make_rot_mat(0.0,0.0,0.0,1.0)

            # w_half = (self.width/self.columns) / 2
            # h_half = (self.height/self.rows) / 2
            w_half = (self.height/self.rows) / 2
            h_half = (self.width/self.columns) / 2

            pos = transform * avango.gua.Vec3(w_half, h_half, 0.0) #* rot
            uv  = avango.gua.Vec2(img.x_max, img.y_min)
            t1_v1 = ImageVertex(self.dynamic_triangle_node, idx * 6, pos, uv)
            
            pos = transform * avango.gua.Vec3(-w_half, -h_half, 0.0)  #* rot
            uv  = avango.gua.Vec2(img.x_min, img.y_max)
            t1_v2 = ImageVertex(self.dynamic_triangle_node, idx * 6 + 1, pos, uv)
            
            pos = transform * avango.gua.Vec3(w_half, -h_half, 0.0)  #* rot
            uv  = avango.gua.Vec2(img.x_max, img.y_max)
            ...
            pos = transform * avango.gua.Vec3(-w_half, h_half, 0.0)  #* rot
            uv  = avango.gua.Vec2(img.x_min, img.y_min )
            ...
            self.min_uv = avango.gua.Vec2(img.x_max, img.y_min)
            self.max_uv = avango.gua.Vec2(img.x_min, img.y_max)
    ...
    def update_images(self, image_id_list, tex_coords):
        ...
        for idx, img_id in enumerate(image_id_list):
            ...
            # uv1  = avango.gua.Vec2(tex_coords[idx][1].x, tex_coords[idx][0].y)
            # self.dynamic_quads[idx][0].update(uv=uv1)
            # uv2  = avango.gua.Vec2(tex_coords[idx][0].x, tex_coords[idx][1].y)
            # self.dynamic_quads[idx][1].update(uv=uv2)
            # uv3  = avango.gua.Vec2(tex_coords[idx][1].x, tex_coords[idx][1].y)
            # self.dynamic_quads[idx][2].update(uv=uv3)
            # uv4  = avango.gua.Vec2(tex_coords[idx][1].x, tex_coords[idx][0].y)
            # self.dynamic_quads[idx][3].update(uv=uv4)
            # uv5  = avango.gua.Vec2(tex_coords[idx][0].x, tex_coords[idx][0].y)
            # self.dynamic_quads[idx][4].update(uv=uv5)
            # uv6  = avango.gua.Vec2(tex_coords[idx][0].x, tex_coords[idx][1].y)
            # self.dynamic_quads[idx][5].update(uv=uv6)
            # uv1  = avango.gua.Vec2(tex_coords[idx][0].y, tex_coords[idx][1].x)
            # self.dynamic_quads[idx][0].update(uv=uv1)
            # uv2  = avango.gua.Vec2(tex_coords[idx][1].y, tex_coords[idx][0].x)
            # self.dynamic_quads[idx][1].update(uv=uv2)
            # uv3  = avango.gua.Vec2(tex_coords[idx][1].y, tex_coords[idx][1].x)
            # self.dynamic_quads[idx][2].update(uv=uv3)
            # uv4  = avango.gua.Vec2(tex_coords[idx][0].y, tex_coords[idx][1].x)
            # self.dynamic_quads[idx][3].update(uv=uv4)
            # uv5  = avango.gua.Vec2(tex_coords[idx][0].y, tex_coords[idx][0].x)
            # self.dynamic_quads[idx][4].update(uv=uv5)
            # uv6  = avango.gua.Vec2(tex_coords[idx][1].y, tex_coords[idx][0].x)
            # self.dynamic_quads[idx][5].update(uv=uv6)
